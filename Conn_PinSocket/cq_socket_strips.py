...
class socket_strip(PartBase):
    ...
    def __init__(self, params):
        ...
        self.make_me = (
            params["type"] == CaseType.THT and params["pin_style"] == PinStyle.STRAIGHT
        )
        ...
        self.first_pin_pos = (
            ...
            self.pin_row_distance / 2.0 - self.pinsocket_offset,
        )

        self.offsets = (
            -self.first_pin_pos[1] - self.pinsocket_offset * 2.0,
    ...
    def makeModelName(self, genericName):
        ...
        return "PinSocket_{0}x{1:02}_P{2:03.2f}mm_{3}{4}".format(
            self.num_pin_rows,
            self.num_pins / self.num_pin_rows,
            self.pin_pitch,
    ...
    def _make_pinpockets(self):
        ...
        first_pin_pos = (
            self.first_pin_pos[0],
            self.first_pin_pos[1] + self.pinsocket_offset,
        )
    ...
    def _make_pinsocket(self):
        ...
        if self.pinsocket_offset > 0.0:
            pinsocket = pinsocket.union(
                cq.Workplane(
                    "XY",
                    origin=(
                        0.0,
                        (self.pinsocket_offset - self.pin_thickness) / 2.0,
                        0.0,
                    ),
                )
                .rect(self.pin_width, self.pinsocket_offset)
                .extrude(self.pin_thickness)
            )
    ...
    def _make_body(self):
        ...
        if self.body_board_distance > 0.0:
            body = (
                ...
                .cutBlind(self.body_board_distance)
            )
...
class angled_socket_strip(socket_strip):
    ...
    def __init__(self, params):
        ...
        self.make_me = (
            params["type"] == CaseType.THT and params["pin_style"] == PinStyle.ANGLED
        )  # and not params.pin_pitch == 1.27
        self.rotation = params["rotation"]
        ...
        self.offsets = (
            0.0 if self.num_pin_rows == 1 else -self.pin_pitch / 2.0,
            -self.first_pin_pos[0],
            0,
        )
        self.translate = (
            0.0,
            self.body_offset + self.pin_row_distance / 2.0,
            self.body_width / 2.0,
        )

    def _make_body(self):
        body = (
            socket_strip._make_body(self)
...
class smd_socket_strip(socket_strip):
    ...
    def __init__(self, params):
        ...
        self.make_me = (
            ...
            and self.num_pins >= 2
            and (self.num_pin_rows == 1 or not self.odd_pins)
        )
        #        self.make_me = self.make_me and (self.pin_pitch != 1.27 or self.num_pin_rows > 1)
        self.rotation = 90
        self.pin_length = (
            (self.pin_length - self.pin_row_distance) / 2.0
            - self.pin_thickness
            - self.pin_thickness / 4.0
        )
    ...
    def makeModelName(self, genericName):
        return socket_strip.makeModelName(self, genericName) + (
            ""
            if self.num_pin_rows == 2
            else ("_Pin1Right" if self.pin1start_right else "_Pin1Left")
        )

    def _make_pins(self):

        pin = (
            self._make_angled_pin(top_length=self.body_board_distance)
            .union(self._make_pinsocket())
        ...
        if self.num_pin_rows == 1:
            pin = pin.translate(
                (
                    self.first_pin_pos[0]
                    + (0.0 if not self.pin1start_right else -self.pin_pitch),
                    self.first_pin_pos[1],
                    0.0,
                )
            )
            if self.odd_pins:
                if self.pin1start_right:
                    odd_pin = pin.rotate((0, 0, 0), (0, 0, 1), 180).translate(
                        (-self.pin_pitch, 0.0, 0.0)
                    )
                else:
