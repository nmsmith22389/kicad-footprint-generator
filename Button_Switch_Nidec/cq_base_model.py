...
class PartBase(object):
    ...
    def __init__(self, params):
        ...
        self.pin_rows_distance = (
            params.pin_rows_distance if "pin_rows_distance" in args else self.pin_pitch
        )
        ...
        self.body_board_distance = (
            params.body_board_distance if "body_board_distance" in args else 0.0
        )
    ...
    def _make_plastic_body(
        ...
        return (
            ...
            .rect(D_b, E1_b)
            .workplane(offset=A2_b, centerOption="CenterOfMass")
            .rect(self.body_length, self.body_width)
            .workplane(offset=pin_area_height, centerOption="CenterOfMass")
            .rect(self.body_length, self.body_width)
            .rect(D_t1, E1_t1)
            .workplane(offset=A2_t, centerOption="CenterOfMass")
            .rect(D_t2, E1_t2)
            .loft(ruled=True)
        )
        ...
        return (
            Polyline(
                cq.Workplane("XZ").workplane(
                    offset=-params["pin_thickness"] / 2.0, centerOption="CenterOfMass"
                )
            )
            .addPoints(
                [
            ...
            .addMirror()
            .make()
            .extrude(params["pin_thickness"])
        )

    def _make_angled_pin(
        self, params, pin_height=None, top_length=0.0, top_extension=0.0, style="SMD"
    ):
        ...
        if style == "SMD":  # make a horizontal pin
            pin = (
                Polyline(
                    cq.Workplane("XY").workplane(
                        offset=-r_lower_o, centerOption="CenterOfMass"
                    ),
                ...
                .addThreePointArc(
                    (params["pin_width"] / 2.0 - d, d),
                    (params["pin_width"] - d * 2.0, 0.0),
                )
                .addPoint(d, -params["pin_length"] + d)
                .make()
                .extrude(params["pin_thickness"])
            ...
            pin = (
                self._make_straight_pin(params, pin_height)
        ...
        arc = (
            Polyline(
                cq.Workplane("YZ").workplane(
                    offset=-params["pin_width"] / 2.0, centerOption="CenterOfMass"
                )
            )
        ...
        if round(top_length, 6) != 0.0:
            pin = pin.union(
                cq.Workplane("XZ")
                .workplane(
                    offset=-params["pin_thickness"] / 2, centerOption="CenterOfMass"
                )
    ...
    def _make_Jhook_pin(self, params, r_upper_i=None, r_lower_i=None):
        ...
        return (
            Polyline(
                cq.Workplane("YZ"), (-(top_length + r_upper_i), params["pin_height"])
            )
            .addPoint(top_length, 0)
            .addArc(r_upper_i, -90)
            .addPoint(
            ...
            .addPoint(
                0,
                params["pin_height"] - r_upper_i - r_lower_i - params["pin_thickness"],
            )
            .addArc(-r_upper_o, -90, 1)
            .addPoint(-top_length, 0)
            .make()
