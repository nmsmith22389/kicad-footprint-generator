...
from .cq_base_parameters import CaseType, PartParametersBase
...
class partParams:
    ...
    def __init__(self, params):
        ...
        self.body_board_distance = (
            params.body_board_distance if "body_board_distance" in args else 0.1
        )

        # Dimensions of pockest at bottom of plastic body
        self.bottom_center_hole_dia = (
            params.bottom_center_hole_dia if "bottom_center_hole_dia" in args else 1.2
        )
        self.bottom_pocket_depth = (
            params.bottom_pocket_depth if "bottom_pocket_depth" in args else 0.15
        )
        self.pocket_edge_pin_length = (
            params.pocket_edge_pin_length if "pocket_edge_pin_length" in args else 1.32
        )
        self.pocket_mid_pin_length = (
            params.pocket_mid_pin_length if "pocket_mid_pin_length" in args else 1.2
        )
        self.pocket_edge_pin_width = (
            params.pocket_edge_pin_width if "pocket_edge_pin_width" in args else 1.72
        )

        # Dimensions of pockest at top of plastic body
        # self.pocket_turn_inset_top_depth: see derived dimensions
        self.pocket_top_side_width = (
            params.pocket_top_side_width if "pocket_top_side_width" in args else 0.45
        )
        self.pocket_top_side_depth = (
            params.pocket_top_side_depth if "pocket_top_side_depth" in args else 0.45
        )

        # Dimensions of plastic body side pads
        self.pad_side_height = (
            params.pad_side_height if "pad_side_height" in args else 2.45
        )
        self.pad_side_length = (
            params.pad_side_length if "pad_side_length" in args else 0.43
        )
        self.pad_side_width = (
            params.pad_side_width if "pad_side_width" in args else 1.05
        )

        # Dimension of metallic cover plate
        self.cover_thickness = (
            params.cover_thickness if "cover_thickness" in args else 0.3
        )
        self.cover_height = params.cover_height if "cover_height" in args else 2.1
        self.cover_side_pocket_width = (
            params.cover_side_pocket_width if "cover_side_pocket_width" in args else 3.1
        )
        self.cover_side_pocket_heigth = (
            params.cover_side_pocket_heigth
            if "cover_side_pocket_heigth" in args
            else 1.45
        )

        # Dimesion of turn inset
        self.turn_inset_height = (
            params.turn_inset_height if "turn_inset_height" in args else 1.3
        )
        self.turn_inset_dia = params.turn_inset_dia if "turn_inset_dia" in args else 3.4
        self.arrowshaft_length = (
            params.arrowshaft_length if "arrowshaft_length" in args else 1.4
        )
        self.arrowshaft_height = (
            params.arrowshaft_height if "arrowshaft_height" in args else 0.7
        )
        self.arrowhead_heigth = (
            params.arrowhead_heigth if "arrowhead_heigth" in args else 0.8
        )
        self.arrowhead_length = (
            params.arrowhead_length if "arrowhead_length" in args else 1.0
        )
        self.arrow_barb_heigth = (
            params.arrow_barb_heigth if "arrow_barb_heigth" in args else 0.45
        )
        self.arrow_pocket_depth = (
            params.arrow_pocket_depth if "arrow_pocket_depth" in args else 1.0
        )

        # Dimension number ring
        self.number_ring_radius = (
            params.number_ring_radius if "number_ring_radius" in args else 2.5
        )
        self.number_font_size = (
            params.number_font_size if "number_font_size" in args else 1.0
        )
        ...
        self.distance_pinTip_rows = (
            params.distance_pinTip_rows if "distance_pinTip_rows" in args else 10.0
        )  # distance from tip of pin from row one to row two
        self.num_pin_rows = params.num_pin_rows if "num_pin_rows" in args else 2
        self.top_length = (
            params.top_length if "top_length" in args else 0.2
        )  # only used for J-hook and angled THT pin

        # Model variants
        self.code_format = (
            params.code_format if "code_format" in args else CodeFormat.REAL
        )
        self.terminal_shape = (
            params.terminal_shape if "terminal_shape" in args else ShapeOfTerminal.GW
        )
        ...
        self.first_pin_pos = (
            self.pin_pitch,
            (self.body_width - self.pocket_edge_pin_width) / 2.0,
        )
        ...
        if paramFunction == None:  # not a valid terminal_shape
            FreeCAD.Console.PrintMessage(
                str(self.terminal_shape)
                + " not a valid member of "
                + str(ShapeOfTerminal)
            )
            self.make_me = False
    ...
    def paramsForJh(self):
        ...
        self.distance_pinTip_rows = (
            7.5  # distance from arc of pin from row one to row two
        )
        self.first_pin_pos = (
            self.pin_pitch * (self.num_pins / 2.0 / self.num_pin_rows - 0.5),
            self.distance_pinTip_rows / 2.0 - self.pin_thickness,
        )
        self.rotation = 90.0
    ...
    def paramsForGw(self):
        ...
        self.body_board_distance = self.pin_thickness - self.bottom_pocket_depth
        self.distance_pinTip_rows = (
            10.0  # distance from tip of pin from row one to row two
        )
    ...
    def paramsForTh(self):
        ...
        self.distance_pinTip_rows = (
            3.0 * self.pin_pitch
        )  # distance from center!!! of pin from row one to row two 3xpitch
        ...
        self.pin_z_shift = (
            self.pin_height + self.pin_thickness - 3.5
        )  # 3.5 is the pin length below body taken from data sheet
        self.rotation = 90.0
...
class variableParams(PartParametersBase):
    ...
    def __init__(self):
        self.base_params = {
            "7010A": self.make_params(
                3.75, CodeFormat.REAL, ShapeOfTerminal.JH, CaseType.SMD
            ),
            "7010B": self.make_params(
                3.75, CodeFormat.REAL, ShapeOfTerminal.GW, CaseType.SMD
            ),
            "7040B": self.make_params(
                3.75, CodeFormat.GRAY, ShapeOfTerminal.GW, CaseType.SMD
            ),
            "7010C": self.make_params(
                3.70, CodeFormat.REAL, ShapeOfTerminal.TH, CaseType.THT
            ),
        }
