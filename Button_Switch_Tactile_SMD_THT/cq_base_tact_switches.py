...
class cqMakerTactSwitch(PartBase, partParamsTactSwitches):
    ...
    def __init__(self, parameter):
        ...
        self.destination_dir = (
            ...
            if parameter["destination_dir"] != None
            else "Button_Switch_SMD.3dshapes"
        )
        self.button_type = (
            parameter["button_type"]
            if parameter["button_type"] != None
            else ButtonType.FLAT
        )  # Type of button from class ButtonType
        self.with_gnd_pin = (
            parameter["with_gnd_pin"] if parameter["with_gnd_pin"] != None else False
        )  # boolean: True if Ground Terminal is present
        self.cover_color_key = (
            parameter["cover_color_key"]
            if parameter["cover_color_key"] != None
            else "metal grey pins"
        )  # Cover color
        self.body_color_key = (
            parameter["body_color_key"]
            if parameter["body_color_key"] != None
            else "black body"
        )  # Body colour
        self.button_color_key = (
            parameter["button_color_key"]
            if parameter["button_color_key"] != None
            else "black body"
        )  # botton color
        self.pin_color_key = (
            parameter["pins_color_key"]
            if parameter["pins_color_key"] != None
            else "metal grey pins"
        )  # Pin color
        self.gnd_pin_tip_to_center = (
            parameter["gnd_pin_tip_to_center"]
            if parameter["gnd_pin_tip_to_center"] != None
            else 4.0
        )  # pos of GND pin rel. to center of body, measured to tip of pin
        self.no_gnd_pins = (
            parameter["no_gnd_pins"] if parameter["no_gnd_pins"] != None else 1
        )  # number of groung pins (1 or 2); default: 1
        self.offset_gnd_pin = (
            parameter["offset_gnd_pin"] if parameter["offset_gnd_pin"] != None else 0.0
        )  # offset of ground pin to middle of housing; default: 0.0
        self.gnd_pin_width = (
            parameter["gnd_pin_width"]
            if parameter["gnd_pin_width"] != None
            else self.pin_width
        )  # offset of ground pin to middle of housing; default: 0.0
        ...
        self.edge_fillet = (
            None  # if not None the outer edges of body and cover will be fillet
        )
        self.edge_chamfer = (
            None  # if not None the outer edges of body and cover will be chamfer
        )
    ...
    def _paramsForB3FS(self):
        ...
        self.button_proj_hight_mid = (
            self.button_height - self.button_proj_hight_bot - self.button_proj_hight_top
        )
    ...
    def _paramsForEVPBF(self):
        ...
        # button has a plate below cover
        self.button_has_base_plate = True
        self.button_base_height = (
            self.cover_pin_side_sheet_height - self.cover_thickness
        )
        self.body_height_plastic -= self.button_base_height
    ...
    def _padsSideCenter(self):
        ...
        pad = (
            cq.Workplane(
                "XY",
                origin=(
                    (self.body_length - self.pad_side_length) / 2.0,
                    0.0,
                    self.body_height_plastic - self.pad_side_height,
                ),
            )
            .rect(self.pad_side_length, self.pad_side_width)
            .extrude(self.pad_side_height)
        )
    ...
    def _padSideEdge(self):
        ...
        pad = (
            cq.Workplane(
                ...
                origin=(
                    (self.body_length - self.pad_side_length) / 2.0,
                    (self.body_width - self.pad_side_width) / 2.0,
                ),
            )
            ...
            .edges(">Z")
            .edges(">X")
            .fillet(self.cover_thickness * 0.99)
        )
    ...
    def __pocketEdgePinBottom(self):
        ...
        return (
            cq.Workplane("XY")
            .rect(
                (body_length_without_pads - self.pin_pocket_edge_length),
                (self.body_width - self.pin_pocket_edge_width),
                forConstruction=True,
            )
            .vertices()
            .rect(self.pin_pocket_edge_length, self.pin_pocket_edge_width)
            .extrude(self.bottom_pocket_depth)
        )
    ...
    def _mountingPads(self, pos, r=0.65):
        ...
        pad = (
            ...
            .circle(r)
            .extrude(2.0 * self.cover_thickness)
            .edges(">Z")
            .fillet(0.05)
    ...
    def _pegs(self):
        ...
        peg = (
            cq.Workplane("XY")
            .circle(self.pegs_radius)
            .extrude(-self.pegs_heigth)
            .edges("<Z")
            .chamfer(self.pegs_radius * 0.3)
        )
    ...
    def makePlasticCase(self):
        ...
        if self.cover_plate_with_side_sheets:
            body_length_without_pads = self.body_length - 2.0 * self.pad_side_length
            body = body.rect(body_length_without_pads, self.body_width).extrude(
                self.body_height_plastic
            )
            ...
            body = body.rect(self.body_length, self.body_width).extrude(
                self.body_height_plastic
            )

            body = body.union(
                self._mountingPads(
                    (self.body_length / 2.0 * 0.7, self.body_width / 2.0 * 0.7),
                    self.body_length * 0.1,
                )
            )
    ...
    def _coverSideSheetsClosed(self):
        ...
        body = (
            Polyline(
                cq.Workplane("YZ").workplane(offset=body_length_without_pads / 2.0)
            )
    ...
    def _coverSideSheetsOpen(self):
        ...
        body = (
            ...
            .addPoint(
                0.0,
                self.cover_side_height
                - self.cover_side_pocket_heigth
                - self.cover_thickness,
            )
            .addPoint(
                -self.cover_side_clips_bottom_width
                + (self.body_width - self.cover_side_pocket_width) / 2.0,
                0.0,
            )
            .make()
            .extrude(self.cover_thickness)
    ...
    def _coverSideSheetsSolid(self):
        ...
        body = (
            cq.Workplane("YZ")
            .workplane(offset=body_length_without_pads / 2.0)
            .box(
                self.cover_solid_side_width,
                self.cover_side_height,
                self.cover_thickness,
            )
            .edges(">Z")
            ...
            .translate(
                (
                    self.cover_thickness / 2.0,
                    0.0,
                    self.body_height - self.cover_side_height / 2.0,
                )
            )
        )
    ...
    def __coverPinSideSheets(self):
        ...
        body = (
            cq.Workplane("XZ")
            .box(
                self.cover_pin_side_sheet_width,
                self.cover_pin_side_sheet_height,
                self.cover_thickness,
            )
            .translate(
                (
                    0.0,
                    (self.body_width + self.cover_thickness) / 2.0,
    ...
    def _buttonRingRound(self):
        ...
        body = (
            cq.Workplane("XY")
            .workplane(offset=self.cover_thickness)
    ...
    def _buttonRingOctagon(self):
        ...
        body = (
            ...
            .extrude(self.button_ring_heigth)
            .edges("|Z")
            .chamfer(self.button_ring_width * 0.2)
            .edges("|Z")
            .fillet(self.button_ring_width * 0.1)
        )
    ...
    def _makeGndPin(self):
        ...
        if self.terminal_shape.startswith("TH"):
            ...
            gndPin = (
                self._make_angled_pin(pl, top_length, 0.0, "THT")
                .rotate((0, 0, 0), (1, 0, 0), -90.0)
                .rotate((0, 0, 0), (0, 0, 1), -90.0)
                .translate(
                    (
                        -self.body_length / 2.0 + self.cover_thickness,
                        0.0,
                        self.body_height
                        - self.cover_thickness
                        + self.pin_thickness / 2.0,
                    )
                )
            )
            ...
            bl = min(
                self.pin_bottom_length, self.pin_length - 2.0 * self.pin_thickness
            )  # 1 x thickness + 2 x radius (1/2 thickness)
            ri = self.pin_thickness / 2.0
            gndPin = (
                self._make_gullwing_pin(
                    self.body_height - self.cover_thickness + self.body_board_distance,
                    bl,
                    ri,
                    ri,
                )
            ...
            gndPin = (
                self._make_Jhook_pin(
                    self.body_height - self.cover_thickness + self.body_board_distance,
                    self.pin_bottom_length / 2.0,
                    top_len,
                    0.01,
                    0.01,
                )
                ...
                .translate(
                    (
                        -(self.gnd_pin_tip_to_center - self.pin_thickness),
                        -self.offset_gnd_pin,
                        -self.body_board_distance,
                    )
                )
    ...
    def _coverCentralHole(self):
        ...
        body = (
            cq.Workplane("XY")
            .circle(self.button_hole_dia / 2.0)
            .extrude(self.cover_thickness + self.button_ring_heigth)
        )
        return body
    ...
    def _buttonBasePlate(self):
        ...
        body = (
            cq.Workplane("XY")
            .workplane(offset=self.body_height - self.cover_thickness)
            .rect(body_length_without_pads + 0.01, self.body_width + 0.01)
            .extrude(-self.button_base_height)
            .edges("|Z")
            .fillet(0.5)
        )
    ...
    def makeButton(self):
        ...
        h = (
            self.button_height
            if self.button_type == ButtonType.FLAT
            else self.button_proj_hight_bot
        )

        body = (
            cq.Workplane("XY")
            .workplane(offset=self.body_height - self.cover_thickness)
            .circle(self.button_dia / 2.0)
            .extrude(h)
        )
        ...
        if self.button_type == ButtonType.PROJ:
            proj = (
                cq.Workplane("XY")
                .workplane(offset=self.body_height - self.cover_thickness + h)
                .rect(self.button_proj_length_mid, self.button_proj_length_mid)
                .extrude(self.button_proj_hight_mid)
                .edges("|Z")
                .fillet(0.05)
            )
    ...
    def _makeGwPin(self):
        ...
        return self._make_angular_gullwing_pin(
            self.pin_height, self.pin_bottom_length, self.pin_gw_angle
        )
    ...
    def _makeThPin(self):
        ...
        """
        return (
            self._make_angled_pin(self.pin_height, self.pin_top_length)
            .rotate((0, 0, 0), (1, 0, 0), -90.0)
    ...
    def _makeThcPin(self):
        ...
        """
        return (
            self._make_angled_clamping_pin(
                self.pin_height,
                self.pin_top_length,
    ...
    def _makeThlPin(self):
        ...
        """
        return (
            self._make_angled_lambda_pin(
                self.pin_height, self.pin_top_length, self.tht_pin_clamp_extension
            )
            .rotate((0, 0, 0), (1, 0, 0), -90.0)
            .translate((0.0, 0.0, self.pin_z_shift))
        )
