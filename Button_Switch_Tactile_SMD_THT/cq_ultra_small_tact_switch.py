...
class cqMakerUltraSmallTactSwitch(cqMakerTactSwitch):
    ...
    def __init__(self, parameter):
        ...
        self.button_side_width = (
            parameter["button_side_width"]
            if parameter["button_side_width"] != None
            else 2.6
        )
        self.button_side_height = (
            parameter["button_side_height"]
            if parameter["button_side_height"] != None
            else 1.4
        )
        self.body_length_with_button = (
            parameter["body_length_with_button"]
            if parameter["body_length_with_button"] != None
            else 4.5
        )
    ...
    def __centralSidePads(self):
        ...
        h = (
            self.body_height
            + self.cover_side_height / 2.0
            - (self.pad_side_height + self.cover_thickness) * 1.5
        )
        body = (
            cq.Workplane("YZ")
            .workplane(offset=body_length_without_pads / 2.0)
            .rect(self.body_width - 3.0 * self.pad_side_width, h)
    ...
    def _coverFilletCentralHole(self, body):
        ...
        body = body.edges(s(">Z") - s(">Y") - s("<Y") - s("<X") - s(">X")).fillet(
            self.cover_thickness * 0.25
        )
    ...
    def _makeSideBottomD(self):
        ...
        body_gp = cq.Workplane("XY").rect(l_gp, w_gp).extrude(h_gp)

        pocket = (
            cq.Workplane("XY")
            .rect(l_gp, 0.3)
            .extrude(h_gp)
            .translate((0.0, 0.4, h_gp / 2.0))
        )
