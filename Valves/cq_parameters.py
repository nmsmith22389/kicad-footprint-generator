...
class cq_parameters_help:
    ...
    def create_pins(
        ...
        if pin_type[0] == "roundtap":
            ...
            pins = (
                cq.Workplane("XZ")
                .workplane(offset=0 - (pin_type[2] / 2.0), centerOption="CenterOfMass")
            ...
            for i in range(1, pin_number):
                ...
                pine = (
                    cq.Workplane("XZ")
                    .workplane(
                        offset=0 - (pin_type[2] / 2.0), centerOption="CenterOfMass"
                    )
                    .polyline(pts, includeCurrent=True)
                    .close()
                    .extrude(pin_type[2])
    ...
    def create_sadle_shield(
        self, origo_x, origo_y, A1, sadle, sadle_hole, sadle_shield, rotation
    ):
        ...
        case = (
            cq.Workplane("XY")
            .workplane(offset=A1 + sadle_z, centerOption="CenterOfMass")
            .moveTo(0, 0)
            .circle(s_d / 2.0, False)
            .extrude(s_h)
        )
    ...
    def create_sadle(self, origo_x, origo_y, A1, D, sadle, sadle_hole, rotation):
        ...
        case21 = (
            cq.Workplane("XY")
            .workplane(offset=A1 + sadle_z - 0.2, centerOption="CenterOfMass")
            .moveTo(0, 0)
            .rect(2.0 * tdx, 2.2 * y4)
            .extrude(sadle_h)
        )
        ...
        case21 = (
            ...
            .workplane(offset=A1 + sadle_z, centerOption="CenterOfMass")
            .moveTo(0, 0)
            .rect(sadle_r1, 2.0 * sadle_r1, centered=False)
            .extrude(sadle_h)
        )
        ...
        case21 = (
            ...
            .workplane(offset=A1 + sadle_z, centerOption="CenterOfMass")
            .moveTo(0, 0)
            .rect(2.0 * sadle_r1, sadle_r1, centered=False)
            .extrude(sadle_h)
        )
        ...
        case21 = (
            cq.Workplane("XY")
            .workplane(offset=A1 + sadle_z, centerOption="CenterOfMass")
            .moveTo(0.0 - sadle_r1 - 4.0, 0)
            .rect(2.0 * sadle_r1, (0.0 - sadle_r1), centered=False)
            .extrude(sadle_h)
        )
        case21 = case21.rotate((0, 0, 0), (0, 0, 1), math.degrees(beta))
        ddtx = ddt * math.cos(math.degrees(beta))
        ...
        case21 = (
            ...
            .workplane(offset=A1 + sadle_z, centerOption="CenterOfMass")
            .moveTo(0, 0)
            .rect(0.0 - sadle_r1 - 4.0, 2.0 * (0.0 - sadle_r1), centered=False)
            .extrude(sadle_h)
        )
        case21 = case21.rotate((0, 0, 0), (0, 0, 1), 90.0 - math.degrees(beta))
        ddtx = ddt * math.cos(90.0 - math.degrees(beta))
        ...
        for n in sadle_hole:
            ...
            case21 = (
                cq.Workplane("XY")
                .workplane(offset=A1 + sadle_z - 0.1, centerOption="CenterOfMass")
                .moveTo(sh_x, 0)
                .circle(sh_d / 2.0, False)
                .extrude(sadle_h + 0.2)
            )
            case2 = case2.cut(case21)
        ...
        case9 = (
            cq.Workplane("XY")
            .workplane(offset=A1 + sadle_z, centerOption="CenterOfMass")
            .moveTo(origo_x, 0.0 - origo_y)
            .circle((D / 2.0) - 0.1, False)
            .extrude(sadle_h)
        )
    ...
    def make_spigot(
        ...
        if pin_spigot != None:
            if pin_spigot[0] == "round" or pin_spigot[0] == "tap":
                ...
                if len(pin_spigot) > 2:
                    pint = (
                        cq.Workplane("XY")
                        .workplane(
                            offset=A1 + ((3 * H) / 4), centerOption="CenterOfMass"
                        )
                        .moveTo(origo_x, 0 - origo_y)
                        .circle(pin_spigot[2] / 2.0, False)
                        .extrude(H)
                    )
                    case1 = case1.cut(pint)
            #
            if pin_spigot[0] == "tap" and len(pin_spigot) > 3:
                ...
                pint = (
                    cq.Workplane("XY")
                    .workplane(offset=A1 + ((3 * H) / 4), centerOption="CenterOfMass")
                    .moveTo(0, 0)
                    .rect(0 - d, pin_spigot[3], centered=False)
                    .extrude(H)
                )
                pint = pint.rotate(
                    (0, 0, 0), (0, 0, 1), math.degrees((5.0 * alpha_delta) / 2.0)
                )
        ...
        if len(pin_top_diameter) > 1:
            ...
            pint = (
                ...
                .moveTo(x1, y1)
                .circle(pin_top_diameter[1] / 2.0, False)
                .extrude(H)
            )
            case1 = case1.cut(pint)
            alpha = alpha + alpha_delta
    ...
    def make_body_with_ring_with_cut(
        self,
        origo_x,
        origo_y,
        A1,
        ...
        case1 = (
            cq.Workplane("XY")
            .workplane(offset=A1 + F1 + 2.0, centerOption="CenterOfMass")
            .moveTo(origo_x, 0 - origo_y)
            .circle(TT / 2.0 + 6.0, False)
            .extrude(H)
        )
    ...
    def make_body_with_ring_cricle(
        self,
        origo_x,
        origo_y,
        A1,
        ...
        case1 = (
            cq.Workplane("XY")
            .workplane(offset=A1 - 0.1, centerOption="CenterOfMass")
            .moveTo(origo_x + tx, 0 - origo_y)
            .circle(2.5 * DDq, False)
            .extrude(H + 0.2)
        )
        case = case.cut(case1)
        #
