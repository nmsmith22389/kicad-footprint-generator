...
class cq_parameters_tube_CK6418():
    ...
    def make_case_top(self, params):
        ...
        B = H * 0.95
        #
        case=cq.Workplane("XY").workplane(offset=A1 + B, centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).circle(W / 8.0, False).extrude(H - B)
...
#        case = case.faces("<Z").shell(0.1)
    ...
    def make_case(self, params):
        ...
        B = H * 0.95
        #
        case = cq.Workplane("XY").workplane(offset=A1, centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).rect(E, W).extrude(B)
    ...
    def make_pins(self, params):
        ...
        #
        # Pin 1
        pins = cq.Workplane("XY").workplane(offset=0, centerOption="CenterOfMass").moveTo(sx, 0 - sy).circle(pin_type[1] / 2.0, False).extrude(0 - pin_type[2])
        pins = pins.faces("<Z").fillet(pin_type[1] / 2.2)
        pins = pins.faces(">Z").fillet(pin_type[1] / 2.2)
        pint = cq.Workplane("XY").workplane(offset=0, centerOption="CenterOfMass").moveTo(0, 0).circle(pin_type[1] / 2.0, False).extrude(A1 + 2)
        ...
        # Pin 2
        sx = sx + dx
        pint = cq.Workplane("XY").workplane(offset=0, centerOption="CenterOfMass").moveTo(sx, 0 - sy).circle(pin_type[1] / 2.0, False).extrude(0 - pin_type[2])
        ...
        # Pin 3
        sx = sx + dx
        pint = cq.Workplane("XY").workplane(offset=0 - pin_type[2], centerOption="CenterOfMass").moveTo(sx, 0 - sy).circle(pin_type[1] / 2.0, False).extrude(pin_type[2] + A1)
        pint = pint.faces("<Z").fillet(pin_type[1] / 2.2)
        pins = pins.union(pint)
        #
        # Pin 4
    ...
    def make_npth_pins(self, params):
        ...
        case = cq.Workplane("XY").workplane(offset=A1, centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).rect(E, W).extrude(0.1)
        case = case.faces("<X").edges(">Y").fillet(ffs)
        case = case.faces("<X").edges("<Y").fillet(ffs)
        case = case.faces(">X").edges(">Y").fillet(ffs)
        case = case.faces(">X").edges("<Y").fillet(ffs)

        pins = cq.Workplane("XY").workplane(offset=A1 + 0.5, centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).circle((W / 2.0) - 0.5, False).extrude(A1 + 2.0)
        
        pint = cq.Workplane("XY").workplane(offset=A1 + 0.5, centerOption="CenterOfMass").moveTo((origo_x - (W / 2.0)) + 3.0, 0 - origo_y).circle(0.5, False).extrude((2.0 * H) / 3.0)
        ...
        pins = pins.union(pint)
        #
        pint = cq.Workplane("XY").workplane(offset=A1 + 0.5, centerOption="CenterOfMass").moveTo(origo_x - 1.0, 0 - origo_y).circle(0.5, False).extrude((2.0 * H) / 3.0)
        pint = pint.faces(">Z").fillet(0.4)
        pins = pins.union(pint)
        #
        pint = cq.Workplane("XY").workplane(offset=A1 + 0.5, centerOption="CenterOfMass").moveTo(origo_x + 1.0, 0 - origo_y).circle(0.5, False).extrude((2.0 * H) / 3.0)
        ...
        pins = pins.union(pint)
        #
        pint = cq.Workplane("XY").workplane(offset=A1 + (H / 6.0), centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).rect(W * 0.8, W / 4.0).extrude(H / 3.0)
