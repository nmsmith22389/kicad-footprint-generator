...
class cq_parameters_tube_CK6418:
    ...
    def make_case_top(self, params):
        ...
        case = (
            cq.Workplane("XY")
            .workplane(offset=A1 + B, centerOption="CenterOfMass")
    ...
    def make_pins(self, params):
        ...
        pint = (
            cq.Workplane("XY")
            .workplane(offset=0 - pin_type[2], centerOption="CenterOfMass")
            .moveTo(sx, 0 - sy)
            .circle(pin_type[1] / 2.0, False)
            .extrude(pin_type[2] + A1)
        )
        pint = pint.faces("<Z").fillet(pin_type[1] / 2.2)
        pins = pins.union(pint)
        #
    ...
    def make_npth_pins(self, params):
        ...
        pint = (
            cq.Workplane("XY")
            .workplane(offset=A1 + 0.5, centerOption="CenterOfMass")
            .moveTo((origo_x - (W / 2.0)) + 3.0, 0 - origo_y)
            .circle(0.5, False)
            .extrude((2.0 * H) / 3.0)
        )
