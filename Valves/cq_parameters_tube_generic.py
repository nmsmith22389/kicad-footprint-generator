...
class cq_parameters_tube_generic():
    ...
    def make_case_top(self, params):
        ...
        case = cq.Workplane("XY").workplane(offset=A1 + H * 0.90 - (D / 4.0), centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).circle(D / 2.0, False).extrude((D / 4.0))
        case = case.faces(">Z").edges(">Y").fillet(D / 4.1)

        case2=cq.Workplane("XY").workplane(offset=A1 + H * 0.90, centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).circle(D / 8.0, False).extrude(A1 + H * 0.10 )
    ...
    def make_case(self, params):
        ...
        ffs = D / 12.0
        case = cq.Workplane("XY").workplane(offset=A1, centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).circle(D / 2.0, False).extrude(H1)
    ...
    def make_pins(self, params):
        ...
        if pin_type[0] == 'round':
            ...
            pins = cq.Workplane("XY").workplane(offset=A1 + 0.1, centerOption="CenterOfMass").moveTo(x1, y1).circle(pin_type[1] / 2.0, False).extrude(A1 - (0.1 + pin_type[2]))
        ...
        if center_pin != None:
            if center_pin[0] == 'metal':
                pint = cq.Workplane("XY").workplane(offset=A1 + 0.1, centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).circle(center_pin[1] / 2.0, False).extrude(A1 - (0.1 + center_pin[2]))
                pint = pint.faces("<Z").fillet(pin_type[1] / 5.0)
                pins  = pins.union(pint)
    ...
    def make_npth_pins(self, params):
        ...
        pins = pins.union(pint)
        #
        pint = cq.Workplane("XY").workplane(offset=A1 + (H / 6.0), centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).rect(D * 0.8, D / 4.0).extrude(H / 3.0)
        ...
        if npth_pin != None:
            if npth_pin[0] == 'metal':
                pint = cq.Workplane("XY").workplane(offset=A1 + 0.1, centerOption="CenterOfMass").moveTo(origo_x, 0 - origo_y).circle(npth_pin[1] / 2.0, False).extrude(0 - (H + 0.1 + pin_type[2]))
                pins  = pins.union(pint)
