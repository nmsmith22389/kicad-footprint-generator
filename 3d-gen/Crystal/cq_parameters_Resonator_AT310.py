...
class cq_parameters_Resonator_AT310():
    ...
    def make_top(self, params):
        ...
        top = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset = tt).moveTo(0.0, 0.0).circle(W / 2.0, False).extrude(lb)
        top = top.faces(">Z").edges().chamfer(lb / 4.0)

        top0 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset = tt).moveTo(0.0, 0.0).circle(W / 2.2, False).extrude(L * (1.0 - tt))
        top = top.union(top0)
