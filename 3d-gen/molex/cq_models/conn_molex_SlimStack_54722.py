...
def generate_contact(series_params, num_pins):
    ...
    ribbon = Ribbon(cq.Workplane("YZ").workplane(offset=-pin_width/2.0 - pin_group_width/2.0, centerOption="CenterOfMass"), c1_list)
    ...
    ribbon = Ribbon(cq.Workplane("YZ").workplane(offset=-pin_width/2.0 - pin_group_width/2.0, centerOption="CenterOfMass"), c2_list)
...
def generate_body(series_params, num_pins):
    ...
    if num_recesses > 0:
        body = body.faces("<Y").workplane(centerOption="CenterOfMass").center(-recess_offset, (body_height - recess_height)/2.0).rarray(recess_pitch, 1, num_recesses, 1).rect(recess_width, recess_height).cutBlind(-recess_depth)
        body = body.faces(">Y").workplane(centerOption="CenterOfMass").center(recess_offset, (body_height - recess_height)/2.0).rarray(recess_pitch, 1, num_recesses, 1).rect(recess_width, recess_height).cutBlind(-recess_depth)

    cutter_A = cq.Workplane("XZ").workplane(offset=body_width/2.0, centerOption="CenterOfMass").center(0, pin_recess_height/2.0).rect(body_length,pin_recess_height).extrude(-body_fillet_radius)
    ...
    pocket_chamfer = cq.Workplane("XY").workplane(offset=body_height, centerOption="CenterOfMass")\
        .rect(body_length - 2.0 * pocket_inside_distance + body_chamfer / 2.0, pocket_width + body_chamfer)\
        .workplane(offset=-body_chamfer, centerOption="CenterOfMass").rect(body_length - 2.0 * pocket_inside_distance, pocket_width)\
        .loft(combine=True)

    body = body.cut(pocket_chamfer)

    island = cq.Workplane("XY").workplane(offset=body_height, centerOption="CenterOfMass")\
    ...
    # contact holes
    body = body.faces(">Z").workplane(centerOption="CenterOfMass").center(0, hole_offset)
    ...
    # notches
    notch1 = cq.Workplane("XY").workplane(offset=body_height, centerOption="CenterOfMass").moveTo(body_length/2.0, 0)\
