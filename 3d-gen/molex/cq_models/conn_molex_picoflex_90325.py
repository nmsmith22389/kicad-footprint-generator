...
def generate_straight_pin(series_params):
    ...
    pin=cq.Workplane("XY").workplane(offset=0, centerOption="CenterOfMass")\
...
def generate_pins(series_params, num_pins):
    ...
    for i in range(0, num_pins):
        ...
        pinTop=cq.Workplane("XY").workplane(offset=-pin_width/2.0, centerOption="CenterOfMass")\
...
def generate_body(series_params, num_pins):
    ...
    body_block=cq.Workplane("XY").workplane(offset=0, centerOption="CenterOfMass")\
...
#    body_block = body_block.faces("<X").edges("<Y").chamfer(series_params['pin_chamfer_short / 2.0,series_params['pin_chamfer_short / 2.0)
    ...
    while (body_y > body_y_end):
        ...
        body = cq.Workplane("XY").workplane(offset=body_block_height - body_height, centerOption="CenterOfMass")\
                .moveTo(body_x, body_y)\
                .rect(body_width, body_length, False)\
                .extrude(body_height)
        body_block = body_block.cut(body)
    ...
    body = cq.Workplane("XY").workplane(offset=(body_block_height / 2.0), centerOption="CenterOfMass")\
        .moveTo(body_x, body_y)\
