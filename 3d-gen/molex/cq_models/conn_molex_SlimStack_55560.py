...
def generate_pin(series_params, num_pins, calc_dim):
    ...
    ribbon = Ribbon(cq.Workplane("YZ").workplane(offset=-pin_width/2.0 - pin_group_width/2.0, centerOption="CenterOfMass"), p_list)
...
def generate_contact(series_params,calc_dim):
    ...
    ribbon = Ribbon(cq.Workplane("YZ").workplane(offset=-contact_width/2.0 - pin_group_width/2.0, centerOption="CenterOfMass"), c_list)
...
def generate_body(series_params,num_pins, calc_dim):
    ...
    body_A = body_A.faces("<Y").workplane(centerOption="CenterOfMass").center(0, -body_height/2.0).rect(body_length-2*pocket_inside_distance,pin_recess_height*2.0).cutBlind(-body_fillet_radius)
    body_A = body_A.faces(">Y").workplane(centerOption="CenterOfMass").center(0, -body_height/2.0).rect(body_length-2*pocket_inside_distance,pin_recess_height*2.0).cutBlind(-body_fillet_radius)
    ...
    pocket = cq.Workplane("XY").workplane(offset=body_height, centerOption="CenterOfMass")\
    ...
    body = body.faces(">Z").workplane(centerOption="CenterOfMass").center(0, top_slot_offset)
    ...
    body = body.faces(">Z").workplane(centerOption="CenterOfMass").center(0, pocket_width / 2.0)
   ...
   # cut overall side profile
    cutter_A = cq.Workplane("YZ").workplane(offset=(body_length - 2.0 * pocket_inside_distance) / 2.0, centerOption="CenterOfMass").center(body_width / 2.0 - 0.2, body_height)\
    ...
    cutter = cq.Workplane("XY").workplane(offset=0.49, centerOption="CenterOfMass").center(0, body_width / 2.0)
    ...
    if 'all' in lock_positions:
        ...
        if 'none' in lock_positions:
            ...
            overcut = [i for i in range(1, 1 + int(num_pins / 2)) if i not in lock_positions]
        cut = cq.Workplane("XY").workplane(offset=0.49, centerOption="CenterOfMass").center(-pin_group_width / 2.0, body_width / 2.0)\
