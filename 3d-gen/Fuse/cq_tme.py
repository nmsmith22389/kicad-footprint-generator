...
class cq_tme():
    ...
    def make_body(self, params, modelID): #make_body_Stelvio_Kontek_PTF78(self, modelID):
        ...
        #
        # Cut the undeside
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0.0).moveTo(0.0, 0.0).rect(W - 6.0, L).extrude(1.0)
        case = case.cut(case1)
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0.0).moveTo(0.0, 0.0).rect(W, L - 2.0).extrude(1.0)
        case = case.cut(case1)
        #
        #
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0.0).moveTo(0.0 - ((W / 2.0) - 0.5), 0.0).rect(1.0, L).extrude(1.0)
        case = case.cut(case1)
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0.0).moveTo((W / 2.0) - 0.5, 0.0).rect(1.0, L).extrude(1.0)
        case = case.cut(case1)
        #
        # Cut top
        #
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=H / 2.0).moveTo(0.0, 0.0).rect(8.0, 5.0).extrude(H)
        case = case.cut(case1)
        #
    ...
    def make_pin(self, params, modelID): #make_pin_Stelvio_Kontek_PTF78(self, modelID):
        ...
        for p in pin:
            ...
            if p[0] == 'rect':
                ...
                case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=2.0).moveTo(x, y).rect(l, w).extrude(0.0 - (h + 2.0))
