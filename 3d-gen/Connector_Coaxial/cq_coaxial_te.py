...
class cq_coaxial_te():
    ...
    def make_top_BNC_TEConnectivity_1478204(self, params):
        ...
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=AZ1).moveTo(AX1, AY1).circle(AW1 / 4.0 - 0.5, False).extrude(AL1 + 1.0)
        case = case.cut(case1)
    ...
    def make_case_BNC_TEConnectivity_1478204(self, params):
        ...
        # Main body
        #
        case = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0.0).moveTo(0.0, 0.0).circle((W / 2.0), False).extrude(L)
        case = case.faces("<Z").chamfer(0.2)
        
        #
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=L - 0.01).moveTo(0.0, 0.0).circle((AW / 2.0), False).extrude(AL)
        case2 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=L - 0.01).moveTo(0.0, 0.0).circle((AW / 2.0) - 0.1, False).extrude(AL)
