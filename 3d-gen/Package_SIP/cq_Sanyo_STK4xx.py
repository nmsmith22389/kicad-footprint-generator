...
class cq_Sanyo_STK4xx():
    ...
    def make_top_Sanyo_STK4xx_59_2(self, params):
        ...
        # Create dummy
        #
        case = cq.Workplane("XY").workplane(offset=A1 + 1.0, centerOption="CenterOfMass").moveTo(0.0, 0.0).circle(0.005, False).extrude(0.001)
    ...
    def make_case_Sanyo_STK4xx_59_2(self, params):
        ...
        # Create body
        #
        case = cq.Workplane("XY").workplane(offset=0.0, centerOption="CenterOfMass").moveTo(0.0, 0.0).rect(W, L).extrude(H)
        #
        # Hole in body
        #
        for ph in BH:
            case1 = cq.Workplane("ZX").workplane(offset=0.0 - L, centerOption="CenterOfMass").moveTo(ph[1], ph[0]).circle(ph[2] / 2.0, False).extrude((2.0 * L))
            case = case.cut(case1)
        #
        # Cut lower part of body
        #
        case1 = cq.Workplane("XY").workplane(offset=0.0 - 1.0, centerOption="CenterOfMass").moveTo(0.0, 0.0).rect(4.0, L + 2.0).extrude(H + 2.0)
...
#        case = case.faces(">Y").edges("<X").fillet(2.0)
    ...
    def make_pins_Sanyo_STK4xx_59_2(self, params):
        ...
        tx = 0.0
        pin = cq.Workplane("XY").workplane(offset=A1 + 0.1, centerOption="CenterOfMass").moveTo(0.0, 0.0).rect(pin_w, pin_l).extrude(0.0 - pin_h)
        for i in range(0, 14):
            tx = tx + 2.54
            pin1 = cq.Workplane("XY").workplane(offset=A1 + 0.1, centerOption="CenterOfMass").moveTo(tx, 0.0).rect(pin_w, pin_l).extrude(0.0 - pin_h)
            pin = pin.union(pin1)
    ...
    def make_case_Sanyo_STK4xx_78_0(self, params):
        ...
        # Cut lower part of body
        #
        case1 = cq.Workplane("XY").workplane(offset=H / 2.0, centerOption="CenterOfMass").moveTo(0.0 - 3.0, 0.0 - (2.0 * L)).rect(6.0, L * 4.0, centered=False).extrude(0.0 - (H + 2.0))
        case1 = case1.rotate((0,0,0), (0,1,0), 10.0)
        case1 = case1.translate((W / 2 - 0.8, 0.0, 0.0))
        case = case.cut(case1)
        #
        case1 = cq.Workplane("XY").workplane(offset=H * 1.5, centerOption="CenterOfMass").moveTo(0.0 + 4.5, 0.0 - (2.0 * L)).rect(6.0, L * 4.0, centered=False).extrude(0.0 - (H + 2.0))
        case1 = case1.rotate((0,0,0), (0,1,0), 0.0 - 10.0)
        case1 = case1.translate((W / 2 - 0.8, 0.0, 0.0))
        case = case.cut(case1)
        #
        ...
        case = case.cut(case1)
        #
        case1 = cq.Workplane("XY").workplane(offset=H * 1.5, centerOption="CenterOfMass").moveTo(0.0 - 10.5, 0.0 - (2.0 * L)).rect(6.0, L * 4.0, centered=False).extrude(0.0 - (H + 2.0))
        case1 = case1.rotate((0,0,0), (0,1,0), 10.0)
        case1 = case1.translate(( 0.0 - (W / 2 - 0.8), 0.0, 0.0))
        case = case.cut(case1)
