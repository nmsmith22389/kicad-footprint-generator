...
def generate_pins(series_params, num_pins):
    odd_pins = [
        cq.Workplane("XY")
        .moveTo(
            i * series_params["half_pitch"]
            - series_params["half_pitch"] * (num_pins - 1) / 2,
    ...
    anchor_pins = [
        cq.Workplane("XY")
        .moveTo(
            d * ((num_pins - 1) * series_params["half_pitch"] / 2 + 0.7),
...
def generate_body(series_params, body_length):
    ...
    return (
        body_outline.extrude(series_params["body_height"])
        .cut(
            cq.Workplane("XY", origin=(0, 0, series_params["body_height"]))
            .moveTo(
                -slot_length / 2,
                series_params["body_width"] - series_params["body_cut_width"],
            )
        ...
        .translate((0, -series_params["body_y_offset"], 0.02))
        .rotate((0, 0, 0), (0, 0, 1), -90)
    )


def generate_latch(series_params, body_length):
    ...
    bar = (
        ...
        .translate(
            (
                -bar_length / 2,
                series_params["body_width"] - series_params["body_y_offset"] - 0.75,
                0,
            )
        )
    )
...
def generate_part(series_params, num_pins):
    body_length = (
        series_params["body_length_padding"] + num_pins * series_params["half_pitch"]
    )
    return (
        generate_pins(series_params, num_pins),
        generate_body(series_params, body_length),
        generate_latch(series_params, body_length),
    )
