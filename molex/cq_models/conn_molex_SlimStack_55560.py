...
__Comment__ = (
    "model description for Molex SlimStack 55560 series connectors using cadquery"
)
...
def dimensions(series_params, num_pins):
    ...
    return calcDim(
        pin_group_width=pin_group_width, length=length, pocket_length=pocket_length
    )


def generate_pin(series_params, num_pins, calc_dim):
    ...
    ribbon = Ribbon(
        cq.Workplane("YZ").workplane(
            offset=-pin_width / 2.0 - pin_group_width / 2.0, centerOption="CenterOfMass"
        ),
        p_list,
    )
...
def generate_pins(series_params, num_pins, calc_dim):
    pin_pitch = series_params["pitch"]
    pin_A = generate_pin(series_params, num_pins, calc_dim)
    pin_B = pin_A.mirror("XZ")
    pin_pair = pin_A.union(pin_B)
    pins = pin_pair
    for i in range(0, int(num_pins / 2)):
        pins = pins.union(pin_pair.translate((i * pin_pitch, 0, 0)))
    return pins


def generate_contact(series_params, calc_dim):
    pin_group_width = calc_dim.pin_group_width
    ...
    c_list = [
        ...
        (
            ...
            {
                "length": body_height
                - pocket_base_thickness
                - contact_minimum_radius
                - contact_thickness / 2.0
            },
        ),
    ...
    ribbon = Ribbon(
        cq.Workplane("YZ").workplane(
            offset=-contact_width / 2.0 - pin_group_width / 2.0,
...
def generate_contacts(series_params, num_pins, calc_dim):
    pin_pitch = series_params["pitch"]
    contact1 = generate_contact(series_params, calc_dim)
...
def generate_body(series_params, num_pins, calc_dim):
    ...
    body_A = (
        ...
        .extrude(body_height)
        .edges("|Z")
        .fillet(body_fillet_radius)
    )
    body_A = (
        body_A.faces("<Y")
        .workplane(centerOption="CenterOfMass")
        .center(0, -body_height / 2.0)
        .rect(body_length - 2 * pocket_inside_distance, pin_recess_height * 2.0)
        .cutBlind(-body_fillet_radius)
    )
    body_A = (
        body_A.faces(">Y")
        .workplane(centerOption="CenterOfMass")
    ...
    body_B = (
        cq.Workplane("XY")
        .rect(body_length - 0.4, body_width - 0.4)
        .extrude(body_height)
    )
    ...
    pocket = (
        ...
        .extrude(-(body_height - pocket_base_thickness))
        .edges("|Z")
    ...
    body = (
        body.faces(">Z")
        .workplane(centerOption="CenterOfMass")
        .center(0, top_slot_offset)
    )
    body = (
        my_rarray(body, pin_pitch, 1, (num_pins / 2), 1)
        .rect(contact_slot_width, 1)
    ...
    body = (
        body.faces(">Z")
        .workplane(centerOption="CenterOfMass")
        .center(0, pocket_width / 2.0)
    )
    body = (
        my_rarray(body, pin_pitch, 1, (num_pins / 2), 1)
        .rect(contact_slot_width, 2 * contact_thickness)
    ...
    cutter_A = (
        cq.Workplane("YZ")
        .workplane(
            offset=(body_length - 2.0 * pocket_inside_distance) / 2.0,
            centerOption="CenterOfMass",
        )
        .center(body_width / 2.0 - 0.2, body_height)
        .line(0.05, -0.1)
        .line(0, -0.1)
        .line(0.05, -0.1)
        .line(0, -0.3)
        ...
        .line(0, body_height)
        .close()
        .extrude(-(body_length - 2.0 * pocket_inside_distance))
    )
    ...
    cutter = (
        cq.Workplane("XY")
        .workplane(offset=0.49, centerOption="CenterOfMass")
        .center(0, body_width / 2.0)
    )
    cutter = (
        my_rarray(cutter, pin_pitch, 1, (num_pins / 2), 1)
    ...
    if "all" in lock_positions:
        ...
        if "none" in lock_positions:
            ...
            overcut = [
                i for i in range(1, 1 + int(num_pins / 2)) if i not in lock_positions
            ]
        cut = (
            cq.Workplane("XY")
            .workplane(offset=0.49, centerOption="CenterOfMass")
            .center(-pin_group_width / 2.0, body_width / 2.0)
            .rect(contact_slot_width, 0.25)
            .center(0, -body_width)
            .rect(contact_slot_width, 0.25)
            .extrude(3)
...
def generate_part(series_params, num_pins):
    calc_dim = dimensions(series_params, num_pins)
    body = generate_body(series_params, num_pins, calc_dim)
    pins = generate_pins(series_params, num_pins, calc_dim)
    contacts = generate_contacts(series_params, num_pins, calc_dim)
