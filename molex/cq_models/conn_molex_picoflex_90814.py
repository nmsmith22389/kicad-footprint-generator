...
def generate_straight_pin(series_params):
    ...
    pin = (
        cq.Workplane("XY")
        .workplane(offset=1.06, centerOption="CenterOfMass")
...
def generate_pins(series_params, num_pins):
    ...
    for i in range(1, num_pins):
        ...
        if (i % 2) == 0:
            ...
            pins = pins.union(
                pin.rotate((0, 0, 0), (0, 0, 1), 180).translate(
                    (2.5, -(i * pin_pitch), 0)
                )
            )
    ...
    for i in range(0, num_pins):
        ...
        pinTop = (
            cq.Workplane("XY")
            .workplane(offset=-pin_width / 2.0 + 1.06, centerOption="CenterOfMass")
...
def generate_body(series_params, num_pins):
    ...
    while body_y > body_y_end:
        ...
        body = (
            cq.Workplane("XY")
            .workplane(
                offset=body_block_height - body_height + 1.06,
    ...
    body = (
        cq.Workplane("XY")
        .workplane(offset=1.566, centerOption="CenterOfMass")
        .moveTo(body_x, body_y)
    ...
    body = (
        cq.Workplane("XY")
        .workplane(offset=(body_block_height / 2.0) + 1.06, centerOption="CenterOfMass")
        .moveTo(body_x, body_y)
        .circle(0.9)
