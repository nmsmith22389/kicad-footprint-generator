...
def generate_pins(series_params, num_pins):
    ...
    add_p_to_chain(pin_points, (0,series_params['pin_back_height']-series_params['pin_tip_chamfer']))
    add_p_to_chain(pin_points, (-series_params['pin_back_pocket'],0))
    add_p_to_chain(pin_points, (0,series_params['pin_back_top']-series_params['pin_back_height']))
    add_p_to_chain(pin_points, (-series_params['pin_protrusion']+series_params['pin_back_pocket'],0))
    pin_points.append((pin_points[4][0],series_params['contact_to_bottom']+series_params['contact_height']))
    add_p_to_chain(pin_points, (-series_params['contact_front_tip'],0))
    add_p_to_chain(pin_points, (0,-series_params['contact_height']))
    pin_points.append((series_params['pin_tip_y']-2*series_params['pin_tip_chamfer']-series_params['pin_radius']-series_params['pin_contact_len'],pin_points[7][1]))
    add_p_to_chain(pin_points, (series_params['pin_radius'],-series_params['pin_radius']))
    pr1=get_third_arc_point2(pin_points[8],pin_points[9])
    pin_points.append((pin_points[9][0],series_params['pin_tip_chamfer']))
    add_p_to_chain(pin_points, (series_params['pin_tip_chamfer'],-series_params['pin_tip_chamfer']))
    ...
    pin = pin.faces("<Y").edges(">Z")\
        .chamfer(series_params['contact_chamfer_depth'], series_params['contact_chamfer_height'])
    pin = pin.faces("<Y").edges("<Z")\
        .chamfer(series_params['contact_chamfer_height'], series_params['contact_chamfer_depth'])
    ...
    mount_pin1 = cq.Workplane("XZ").workplane(offset=-series_params['mount_back'], centerOption="CenterOfMass")\
    ...
    mount_pin2 = cq.Workplane("XZ").workplane(offset=-series_params['mount_back'], centerOption="CenterOfMass")\
        .moveTo(*mount_pin_points[0]).lineTo(*mount_pin_points[1])\
        .lineTo(*mount_pin_points[2]).threePointArc(arc_points[0],mount_pin_points[3])\
        .lineTo(*mount_pin_points[4]).threePointArc(arc_points[1],mount_pin_points[5])\
        .lineTo(*mount_pin_points[6]).lineTo(*mount_pin_points[7])\
...
def generate_body(series_params, num_pins):
    ...
    body = cq.Workplane("XY").workplane(centerOption="CenterOfMass")\
        .moveTo(0, series_params['body_center_y']).rect(body_len, series_params['body_width'])\
            .extrude(series_params['body_height'])
    body = body.faces("<Z").workplane(centerOption="CenterOfMass")\
        .rect(body_len-2*series_params['body_support_width'],series_params['body_width'])\
        .cutBlind(-series_params['body_main_z'])

    front_face = -series_params['body_center_y']+series_params['body_width']/2
    cutout = cq.Workplane("XZ").workplane(offset=front_face, centerOption="CenterOfMass")\
        .moveTo(0,series_params['body_height']-series_params['body_cutout_depth']/2.0+(series_params['body_top_width']-series_params['body_bottom_width'])/2.0)\
        .rect(body_len-2*series_params['body_side_width'], series_params['body_cutout_h'])\
    ...
    cutout = cq.Workplane("XZ").workplane(offset=front_face, centerOption="CenterOfMass")\
        ...
        .faces(">Y").workplane(centerOption="CenterOfMass").rect(body_front_cutout_len,series_params['body_top_width'])\
    ...
    body = body.edges(BS(
        ...
        (body_len/2.0-series_params['body_side_width']/2.0,
         -front_face+0.1,
         (series_params['body_bottom_width']-series_params['body_main_z'])/2),True))\
    ...
    if num_pins > 3:
        back_cutout_center_x = body_len/2-series_params['back_cutout_center_to_side']

    back_cutout = cq.Workplane("XY").workplane(series_params['body_main_z'], centerOption="CenterOfMass")\
        .moveTo(back_cutout_center_x-series_params['back_cutout_t_width']/2,-front_face)\
        ...
        .hLine((series_params['back_cutout_b_width']-series_params['back_cutout_t_width'])/2.0)\
        .close().extrude(series_params['body_bottom_width']-series_params['body_main_z'])

    if num_pins > 3:
        back_cutout = back_cutout.union(back_cutout.translate((-body_len+2*series_params['back_cutout_center_to_side'],0,0)))
    ...
    add_p_to_chain(poly_points, (0,series_params['mount_holder_top_z']-series_params['mount_holder_chamfer']-series_params['mount_pin_thickness']))
    add_p_to_chain(poly_points, (-series_params['mount_holder_len']+series_params['mount_holder_chamfer'],0))
    add_p_to_chain(poly_points, (-series_params['mount_holder_chamfer'],series_params['mount_holder_chamfer']))
    mount_holder1 = cq.Workplane("XZ").workplane(-series_params['mount_back'], centerOption="CenterOfMass")\
        .moveTo(*sp)\
        .polyline(poly_points, includeCurrent=True).close().extrude(series_params['mount_holder_width'])

    poly_points=mirror(poly_points)
    mount_holder2 = cq.Workplane("XZ").workplane(-series_params['mount_back'], centerOption="CenterOfMass")\
        .moveTo(-sp[0],sp[1])\
        .polyline(poly_points, includeCurrent=True).close().extrude(series_params['mount_holder_width'])


    poly_points=[v_add(sp,(series_params['mount_pin_thickness'],0))]
    add_p_to_chain(poly_points, (0,series_params['mount_pin_height']-series_params['mount_pin_bend_radius']-series_params['mount_pin_thickness']-series_params['mount_pin_thickness']))
    ...
    mount_holder1_cutout = cq.Workplane("XZ").workplane(-series_params['mount_back'], centerOption="CenterOfMass")\
       .moveTo(*sp).polyline(poly_points, includeCurrent=True).close().extrude(series_params['mount_pin_width'])

    poly_points=mirror(poly_points)
    mount_holder2_cutout = cq.Workplane("XZ").workplane(-series_params['mount_back'], centerOption="CenterOfMass")\
        .moveTo(-sp[0],sp[1])\
    ...
    first_cutout_x = (num_pins-1)/2.0*series_params['pin_pitch']+series_params['pin_pitch']
    top_cutout = cq.Workplane("XY").workplane(offset=series_params['body_height'], centerOption="CenterOfMass")\
        .moveTo(first_cutout_x, -front_face+series_params['body_width']-series_params['top_cutout_width']/2.0)\
        .rect(series_params['top_cutout_len'],series_params['top_cutout_width']).extrude(-series_params['top_cutout_depth'])
    top_cutouts = top_cutout
    for i in range(0,num_pins+2):
        top_cutouts=top_cutouts.union(top_cutout.translate((-i*series_params['pin_pitch'],0,0)))
