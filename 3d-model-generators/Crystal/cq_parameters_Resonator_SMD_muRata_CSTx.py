...
class cq_parameters_Resonator_SMD_muRata_CSTx:
    ...
    def make_top(self, params):  # Formerly make_case_top
        ...
        if TW > 0.01:
            ...
            case = (
                cq.Workplane("XY")
                .workplane(centerOption="CenterOfMass", offset=A1 + 0.1)
                .moveTo(origo_x, 0 - origo_y)
                .rect(0.1, 0.1)
                .extrude(0.1)
            )
    ...
    def make_pins(self, case, params):
        ...
        if pin_cut > 0.01:
            pinc = (
                cq.Workplane("XY")
                .workplane(centerOption="CenterOfMass", offset=A1)
                .moveTo(origo_x, (0.0 - origo_y) - ((E + 0.05) / 2.0))
                .circle(pin_cut / 2.0, False)
                .extrude(H + 0.1)
            )
            pins = pins.cut(pinc)
            case = case.cut(pinc)
