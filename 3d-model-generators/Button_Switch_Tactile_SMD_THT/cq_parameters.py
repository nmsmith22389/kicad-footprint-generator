...
class partParamsTactSwitches:
    ...
    def __init__(self, params):
        ...
        )  # side of part where the pins are
        self.body_width = (
            params["body_width"]
            if "body_width" in args and params["body_width"] != None
            else 6.0
        )
        self.body_height_with_button = (
            params["body_height_with_button"]
            if "body_height_with_button" in args
            and params["body_height_with_button"] != None
            else 5.0
        )
        ...
        self.body_board_distance = (
            params["body_board_distance"]
            if "body_board_distance" in args and params["body_board_distance"] != None
            else None
        )
        # if case has alignment pegs 'has_pegs' must be true
        self.has_pegs = (
            params["has_pegs"]
            if "has_pegs" in args and params["has_pegs"] != None
            else False
        )
        self.pegs_pitch = (
            params["pegs_pitch"]
            if "pegs_pitch" in args and params["pegs_pitch"] != None
            else 9.0
        )
        self.pegs_radius = (
            params["pegs_radius"]
            if "pegs_radius" in args and params["pegs_radius"] != None
            else 1.0
        )
        self.pegs_heigth = (
            params["pegs_heigth"]
            if "pegs_heigth" in args and params["pegs_heigth"] != None
            else 1.5
        )
        ...
        self.button_dia = (
            params["button_dia"]
            if "button_dia" in args and params["button_dia"] != None
            else 3.5
        )
        self.button_ring = (
            params["button_ring"]
            if "button_ring" in args and params["button_ring"] != None
            else ButtonRing.NONE
        )
        self.button_ring_radius = (
            params["button_ring_radius"]
            if "button_ring_radius" in args and params["button_ring_radius"] != None
            else 2.35
        )
        self.button_ring_heigth = (
            params["button_ring_heigth"]
            if "button_ring_heigth" in args and params["button_ring_heigth"] != None
            else 1.6
        )
        self.button_ring_length = (
            params["button_ring_length"]
            if "button_ring_length" in args and params["button_ring_length"] != None
            else 3.7
        )
        self.button_ring_width = (
            params["button_ring_width"]
            if "button_ring_width" in args and params["button_ring_width"] != None
            else 3.7
        )
        ...
        self.distance_pinTip_rows = (
            params["distance_pinTip_rows"]
            if "distance_pinTip_rows" in args and params["distance_pinTip_rows"] != None
            else 10.0
        ...
        self.top_length = (
            params["top_length"]
            if "top_length" in args and params["top_length"] != None
            else 0.2
        )  # only used for J-hook and angled THT pin
        self.pin_length_below_body = (
            params["pin_length_below_body"]
            if "pin_length_below_body" in args
            and params["pin_length_below_body"] != None
            else 3.5
        )  # only used for THT pin

        # Model variants
        self.terminal_shape = (
            params["terminal_shape"]
            if "terminal_shape" in args and params["terminal_shape"] != None
            else ShapeOfTerminal.GW
        )
        ...
        self.cover_thickness = (
            params["cover_thickness"]
            if "cover_thickness" in args and params["cover_thickness"] != None
            else 0.3
        )
        self.cover_side_height = 2.1
        self.cover_plate_with_side_sheets = (
            False  # must be true if cover of switch has side clips
        )
        ...
        self.button_height = (
            self.body_height_with_button - self.body_height + self.cover_thickness
        )  # the button heigth is measured from below cover to top
    ...
    def paramsForJh(self):
        ...
        )  # inner radius at top of j-hook pin
        if self.body_board_distance is None:
            self.body_board_distance = max(
                self.pin_thickness - self.bottom_pocket_depth, 0.0
            )
        self.first_pin_pos = (
    ...
    def paramsForTh(self):
        ...
        self.pin_z_shift = (
            self.pin_height - self.pin_thickness / 2.0 - self.pin_length_below_body
        )
        self.rotation = 90.0
