...
class cqMakerCuKPTS125SxTactSwitch(cqMakerTactSwitch):
    ...
    def __padsBottom(self):
        ...
        pad = (
            Polyline(
                cq.Workplane("XY"),
                origin=(-self.pin_pitch / 2.0 - 1.8, self.body_width / 2.0 - 0.5),
            )
            .addPoint(1.8, 0.0)
            .addPoint(0.0, -3.0)
            .addPoint(-3.0, 0.0)
            .addPoint(0.0, 1.8)
            .make()
            .extrude(self.pad_bottom_height)
        )
    ...
    def makePlasticCase(self):
        ...
        body = (
            cq.Workplane("XY")
            .rect(self.body_length, self.body_width)
            .extrude(self.body_height_plastic - self.pad_bottom_height)
        ...
        body = body.union(
            self._mountingPads(
                (self.cover_length / 2.0 * 0.8, self.cover_width / 2.0 * 0.8), 0.65
            )
        )
        ...
        if self.has_pegs:
            body = body.union(
                self._pegs().translate((0.0, 0.0, self.pad_bottom_height))
            )
    ...
    def __pocketCoverSide(self):
        ...
        pocket = (
            cq.Workplane("XY")
            .box(l, w, 2.0 * self.cover_thickness)
    ...
    def makeCoverPlate(self):
        ...
        body = (
            cq.Workplane("XY")
            .rect(self.cover_length, self.cover_width)
            .circle(self.button_dia * 0.52)
            .extrude(self.cover_thickness)
        )
        ...
        if not (
            self.button_type == ButtonType.FLAT and self.terminal_shape.startswith("TH")
        ):
