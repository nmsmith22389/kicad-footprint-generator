...
__Comment__ = (
    "model description for Molex SlimStack 54722 series connectors using cadquery"
)
...
def dimensions(series_params, num_pins):
    ...
    rib_group_outer_width = (
        pin_group_width + series_params["pitch"] + series_params["rib_width"]
    )
    ...
    return calcDim(
        pin_group_width=pin_group_width,
        length=length,
        pocket_length=pocket_length,
        island_length=island_length,
        rib_group_outer_width=rib_group_outer_width,
        slot_width=slot_width,
        num_recesses=num_recesses,
        recess_offset=recess_offset,
    )


def generate_contact(series_params, num_pins):
    ...
    c2_list = [
        (
            "start",
            {
                "position": (
                    (
                        -hole_offset - hole_length / 2.0 + pin_thickness / 2.0,
...
def generate_contacts(series_params, num_pins):
    pitch = series_params["pitch"]
    contact1 = generate_contact(series_params, num_pins)
...
def generate_body(series_params, num_pins):
    ...
    if num_recesses > 0:
        body = (
            ...
            .center(-recess_offset, (body_height - recess_height) / 2.0)
            .rarray(recess_pitch, 1, num_recesses, 1)
    ...
    cutter_A = (
        cq.Workplane("XZ")
        .workplane(offset=body_width / 2.0, centerOption="CenterOfMass")
        .center(0, pin_recess_height / 2.0)
        .rect(body_length, pin_recess_height)
        .extrude(-body_fillet_radius)
    )
    ...
    pocket_chamfer = (
        ...
        .workplane(offset=body_height, centerOption="CenterOfMass")
        .rect(
            body_length - 2.0 * pocket_inside_distance + body_chamfer / 2.0,
            pocket_width + body_chamfer,
        )
        .workplane(offset=-body_chamfer, centerOption="CenterOfMass")
        .rect(body_length - 2.0 * pocket_inside_distance, pocket_width)
        .loft(combine=True)
    )
    ...
    # contact holes
    body = (
        body.faces(">Z").workplane(centerOption="CenterOfMass").center(0, hole_offset)
    )

    body = (
        my_rarray(body, pitch, 1, (num_pins / 2), 1)
    ...
    ribs = (
        my_rarray(ribs, pitch, pocket_width + body_chamfer, (num_pins / 2) + 1, 2)
        .rect(rib_width, rib_depth)
        .extrude(body_height)
        .faces(">Z")
        .edges("|X")
        .fillet((rib_depth - 0.001) / 2.0)
    )

    body = body.union(ribs)

    # slots for contacts
    slot_cutter = cq.Workplane("XY").center(
        -slot_width / 2.0, (island_width / 2.0) - slot_depth
    )

    slot_cutter = (
        my_rarray(slot_cutter, pitch, 1, num_pins / 2, 1)
        .rect(
            slot_width, slot_depth + (pocket_width - island_width) / 2.0, centered=False
        )
        .extrude(slot_height)
    ...
    notch1 = (
        cq.Workplane("XY")
        .workplane(offset=body_height, centerOption="CenterOfMass")
        .moveTo(body_length / 2.0, 0)
        .rect(1, notch_width)
        .extrude(-notch_depth)
        .faces("<Z")
        .edges("<Z")
        .chamfer(notch_depth - 0.001)
    )
...
def generate_part(series_params, num_pins):
    ...
    pins = generate_contacts(series_params, num_pins)
