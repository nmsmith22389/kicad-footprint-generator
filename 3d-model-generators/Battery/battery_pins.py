...
def make_pins(params):
        ...
        if n[0] == "tht":
            ...
            if n[3] == "rect":
                ...
                if len(n) > 6:
                    ...
                    pine = (
                        cq.Workplane("XY")
                        .workplane(offset=A1 + A11 - 0.1)
                        .moveTo(xx, yy)
                        .circle(dhl2 / 2.0, False)
                        .extrude(0.3)
                    )
                    pint = pint.cut(pine)
    ...
    if topear != None:
        for n in topear:
            ...
            pine = (
                cq.Workplane("YZ")
                .workplane(offset=xx - 2.0)
                .moveTo(yy, A1 + A11 + H + th - (tw / 1.5))
                .circle(tw / 4.0, False)
                .extrude(4.0)
            )
            pint = pint.cut(pine)
...
def make_npthpins_S2(params):
    ...
    pint = (
        ...
        .workplane(offset=A1 + A11)
        .moveTo(x1 + x, y1 + y)
        .circle(pinradie, False)
        .extrude(0.0 - pinlength)
    )
