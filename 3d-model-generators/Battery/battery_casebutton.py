...
def make_case_Button1(params):
    ...
    case2 = (
        cq.Workplane("XY")
        .workplane(offset=A1 + A11 + 1.0)
        .moveTo(sx1 + 1.0, sy1 + (sw - 1.0))
        .rect(x1, 0.0 - (sw - 2.0), centered=False)
        .extrude(hh + 2.0)
    )
    incase1 = (
        cq.Workplane("XY")
    ...
    case1 = (
        cq.Workplane("XY").workplane(offset=0.0).polyline(pts).close().extrude(hh + 2.0)
    )
...
def make_case_Button2(params):
    ...
    case1 = (
        cq.Workplane("XY")
        .workplane(offset=A1 + A11)
        .moveTo(0.0 - (L / 2.0), 0.0 - (W / 2.0))
        .rect(L, 0.0 - W, centered=False)
        .extrude(H + 2.0)
    )
    case = case.cut(case1)
    ...
    case1 = (
        cq.Workplane("XY").workplane(offset=0.0).polyline(pts).close().extrude(H + 2.0)
    )
    case1 = case1.translate(
        (0.0 - (ll * 1.25), 0.0 - ((W / 2.0) + (ll / 2.0)), A1 + A11)
    )
    case = case.cut(case1)
