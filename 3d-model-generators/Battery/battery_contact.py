...
def make_battery_contact_BC1(params):
    ...
    pint = pint.translate((TX - 0.01, 1.4, A1 + A11 + 1.0))
...
def make_battery_contact_BC2(params):
    ...
    LC = params[
        "LC"
    ]  # Large circle [x pos, y pos, outer diameter, inner diameter, height]
    BC = params["BC"]  # Blend height
    A1 = params["A1"]  # package board seperation
    # pins = params.pins                  # Pins tht/smd, x pos, y pos, 'round/rect', diameter/x size, y size, length
    # cellsize = params.cellsize           # Battery type
    ...
    pint = (
        cq.Workplane("XY")
        .workplane(offset=A1 + A11 + 1.0)
        .moveTo(x1, y1)
        .circle((id / 2.0) * 0.7, False)
        .extrude(0.2)
    )
    pint1 = (
        cq.Workplane("XY")
        .workplane(offset=A1 + A11 + 1.0 - 0.2)
        .moveTo(x1, y1)
        .circle(tdd, False)
        .extrude(0.2 + 0.4)
    )
    ...
    for i in range(0, cellcnt):
        ...
        while sd > gt:
            if x1 > 0.0:
                ...
                pine = (
                    cq.Workplane("ZY")
                    .workplane(offset=x1 - 0.2)
                    .moveTo(z1, y1)
                    .circle(sd - gt - 0.1, False)
                    .extrude(tx + 0.2)
                )
            else:
                pinf = (
...
def make_battery_contact_BC5(params):
    ...
    pint = (
        cq.Workplane("XY")
        .workplane(offset=A1 + A11)
        .moveTo(0.0 - ((id / 2.0) - 0.3), 0.0)
        .rect(0.1, 3.0)
        .extrude(H - 0.2)
    )
    pine = (
        cq.Workplane("XY")
        .workplane(offset=A1 + A11 + (H - 0.2))
        .moveTo(0.0 - ((id / 2.0) - 0.8), 0.0)
        .rect(1.0, 3.0)
        .extrude(0.1)
    )
...
def make_battery_contact_BC7(params):
    ...
    pine = (
        ...
        .workplane(offset=A1 + A11 + (H - 0.4))
        .moveTo(hx1, hy1)
        .rect(ddx - 0.1, hd1 - 0.1, centered=False)
        .extrude(H)
    )
    pint = pint.cut(pine)
    #
