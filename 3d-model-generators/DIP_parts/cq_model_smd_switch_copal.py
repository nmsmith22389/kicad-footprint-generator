...
class dip_switch_copal_CHS_B(dip_smd_switch_lowprofile):
    ...
    def _make_switchpockets(self):
        ...
        pocket = (
            cq.Workplane("XY", origin=(x0, 0.0, z0))
        ...
        pocket.edges(
            BS(
                (x0 + x2 + 0.1, -w2 - 0.1, z0 - 0.1),
                (x0 - x2 - 0.1, -w2 + 0.1, z0 + 0.1),
            )
        ).chamfer(self.button_pocket_dept - 0.01)
        pocket.edges(
            BS((x0 + x2 + 0.1, w2 - 0.1, z0 - 0.1), (x0 - x2 - 0.1, w2 + 0.1, z0 + 0.1))
        ).chamfer(self.button_pocket_dept - 0.01)
    ...
    def make_buttons(self):
        ...
        button = (
            ...
            .center(
                0,
                -self.button_base / 2.0
                + self.button_length / 2.0
                + self.button_pocket_dept,
            )
            .rect(self.button_width, self.button_length)
            .extrude(self.button_heigth + 0.1)
        )
...
class dip_switch_copal_CVS(dip_switch_copal_CHS_B):
    ...
    def _make_buttonsrecess(self):
        ...
        return (
            ...
            .rect(self.body_length - 1.6, self.body_width - self.button_base)
            .extrude(depth)
            .faces("<Z")
            .edges()
            .chamfer(-depth - 0.01)
    ...
    def make_body(self):
        body = (
            super(dip_switch_copal_CVS, self)
            .make_body(0.2, 0.0)
            .cut(self._make_buttonsrecess())
        )
    ...
    def make_pins(self):
        ...
        case = (
            cq.Workplane(
                ...
                origin=(
                    0.0,
                    self.body_width / 2.0 - self.pin_width + 0.1,
                    self.body_height,
                ),
            )
            .rect(self.body_length, 2.0 * self.pin_width)
            .extrude(self.pin_thickness / 2.0)
        )
        ...
        case1 = (
            cq.Workplane(
                "XY",
                origin=(
                    (self.body_length / 2.0) - (self.pin_width / 2.0),
                    (self.body_width / 2.0) - (self.pin_thickness / 2.0) + 0.1,
                    self.body_height,
                ),
            )
            .rect(self.pin_width, self.pin_thickness)
            .extrude(0.0 - self.body_height)
        )
        ...
        case1 = (
            cq.Workplane(
                ...
                origin=(
                    (self.body_length / 2.0) - (self.pin_width / 2.0),
                    (0.0 - ((self.body_width / 2.0) - (self.pin_thickness / 2.0)))
                    - 0.1,
                    self.body_height,
                ),
            )
        ...
        case1 = (
            cq.Workplane(
                ...
                origin=(
                    0.0,
                    (self.body_width / 2.0) - (self.pin_thickness / 2.0) + 0.1,
                    (4.0 * self.body_height) / 5.0,
                ),
            )
            .rect(self.body_length, self.pin_thickness)
            .extrude(self.body_height / 5.0)
        )
        case = case.union(case1)
