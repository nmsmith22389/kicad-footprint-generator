...
def generate_straight_pins(params):
    ...
    signal_pin = (
        cq.Workplane("YZ")
        .workplane(-pin_distance / 2 - pin_width / 2, centerOption="CenterOfMass")
        .move(-2.4754, 0)
        .vLine(0.3)
        .hLine(0.7)
        .vLine(0.35)
...
def generate_angled_pins(params):
    ...
    pins = pins.union(
        mount_pin.translate((-(num_pins - 1) * pin_pitch / 2 - 1.85, 0, 0))
    )
    pins = pins.union(
        mount_pin.translate(((num_pins - 1) * pin_pitch / 2 + 1.85, 0, 0))
    )
...
def generate_straight_body(params):
    ...
    R_top_side_L_cut = (
        cq.Workplane("YZ")
        .workplane(-body_length / 2, centerOption="CenterOfMass")
        .move(-body_width / 2, body_height)
        .hLine(top_L_side_cut_width)
        .vLine(-top_L_side_cut_short_height)
        .hLine(-(top_L_side_cut_width - top_L_side_cut_short_width))
        .vLine(-(top_L_side_cut_height - top_L_side_cut_short_height))
        .hLine(-top_L_side_cut_short_width)
        .close()
    ...
    front_U_cut = (
        cq.Workplane("XY")
        .workplane(body_height, centerOption="CenterOfMass")
        .move(front_U_width / 2, -body_width / 2 + front_U_y_offset)
        .vLine(front_U_height)
        .hLine(-front_U_short_width)
        .vLine(-front_U_short_height)
        .hLine(
            -(
                front_U_width
                - 2 * front_U_short_width
                - (num_pins - 1) * pin_pitch
                - front_U_pin_nobs_width
            )
            / 2
        )
    )
    for x in range(0, num_pins):
        front_U_cut = (
            front_U_cut.vLine(-front_U_pin_nobs_height)
            .hLine(-front_U_pin_nobs_wall_width)
            .vLine(front_U_pin_nobs_height)
            .hLine(-pin_width)
            .vLine(-front_U_pin_nobs_height)
            .hLine(-front_U_pin_nobs_wall_width)
            .vLine(front_U_pin_nobs_height)
    ...
    lock_tab = (
        cq.Workplane("YZ")
        .workplane(-lock_tab_width[num_pins] / 2, centerOption="CenterOfMass")
        .move(lock_tab_y_offset, lock_tab_z_offset)
        .hLine(lock_tab_depth)
        .line(-lock_tab_depth, lock_tab_height)
        .close()
        .extrude(lock_tab_width[num_pins])
    )

    body_top_square_hole_template = (
        cq.Workplane("XY")
        .workplane(centerOption="CenterOfMass")
        .box(
            body_top_square_hole_width,
            body_top_square_hole_height,
            body_top_square_hole_depth,
            centered=(True, True, False),
        )
        .translate(
            (
                -(body_length - body_top_square_hole_width) / 2
                + body_top_square_hole_x_offset,
                (body_width - body_top_square_hole_height) / 2
                - body_top_square_hole_y_offset,
                body_height - body_top_square_hole_depth,
            )
        )
    )

    body_top_square_hole = body_top_square_hole_template.union(
        body_top_square_hole_template.translate(
            (
                body_length
                - body_top_square_hole_width
                - body_top_square_hole_x_offset * 2,
                0,
                0,
            )
        )
    ...
    if 10 <= num_pins <= 15:
        body_top_square_hole = body_top_square_hole.union(
            body_top_square_hole_template.translate(
                (-body_top_square_hole_width - body_top_square_hole_x_offset, 0, 0)
            )
        )
        body_top_square_hole = body_top_square_hole.union(
            body_top_square_hole_template.translate(
                (
                    (
                        -body_length
                        + body_top_square_hole_width * 2
                        + body_top_square_hole_x_offset * 3
                    ),
                    0,
                    0,
                )
