...
def generate_pinpockets(params, part_params):
    ...
    if params["orientation"] == "Horizontal":
        if params["series_prefix"] == "SPT 1.5/":
            ...
            rpocket = (
                cq.Workplane("XZ", origin=(0, 0) + (3.35,))
                .rect(2.25, 2.1)
                .extrude(-3.0)
            )
        if params["series_prefix"] == "SPT 2.5/":
            ...
            rpocket = (
                cq.Workplane("XZ", origin=(0, 0) + (3.85,)).rect(3.7, 2.4).extrude(-3.3)
            )
        if params["series_prefix"] == "SPT 5/":
            ...
            rpocket = (
                cq.Workplane("XZ", origin=(0, 0) + (6.65,)).rect(3.7, 3.9).extrude(-6)
            )
    else:
        if params["series_prefix"] == "SPT 1.5/":
            pocket = (
                cq.Workplane("XZ", origin=(0, 0) + (params["height"],))
                .lineTo(1.6, 0)
                .lineTo(1.6, -2.1)
                .lineTo(1.25, -3.0)
                .lineTo(0, -3.0)
            ...
            rpocket = (
                cq.Workplane("XY", origin=(0, 3.35) + (params["height"],))
                .rect(2.25, 2.1)
                .extrude(-3.0)
            )
        if params["series_prefix"] == "SPT 2.5/":
            ...
            rpocket = (
                cq.Workplane("XY", origin=(0, 3.85) + (params["height"],))
                .rect(3.7, 2.4)
                .extrude(-3.3)
            )
        if params["series_prefix"] == "SPT 5/":
            ...
            rpocket = (
                cq.Workplane("XY", origin=(0, 6.65) + (params["height"],))
                .rect(3.7, 3.9)
                .extrude(-6)
            )
            ...
            if params["series_prefix"] == "SPT 1.5/":
                objs.append(
                    pocket.translate(
                        (
                            -(
                                part_params["pins"] * params["pitch"]["x"]
                                + params["fab"]["right"]
                            )
                            / 2
                            + params["pitch"]["x"] / 2
                            + i * params["pitch"]["x"],
                            -(params["fab"]["bottom"] - params["fab"]["top"]) / 2,
                            3.8,
                        )
                    )
                )
                ...
                objs.append(
                    pocket.translate(
                        (
                            ...
                            + params["pitch"]["x"] / 2
                            + i * params["pitch"]["x"],
                            -(params["fab"]["bottom"] - params["fab"]["top"]) / 2 + 3.8,
                            0,
                        )
                    )
                )
                ...
                objs.append(
                    pocket.translate(
                        (
                            ...
                            + params["pitch"]["x"] / 2
                            + i * params["pitch"]["x"],
                            -(params["fab"]["bottom"] - params["fab"]["top"]) / 2 + 4.1,
                            0,
                        )
                    )
                )
                ...
                objs.append(
                    pocket.translate(
                        (
                            ...
                            + params["pitch"]["x"] / 2
                            + i * params["pitch"]["x"],
                            -(params["fab"]["bottom"] - params["fab"]["top"]) / 2 + 3.7,
                            0,
                        )
                    )
                )
...
def generate_body(params, part_params):
    ...
    body = (
        cq.Workplane("XY")
        .rect(
            params["pitch"]["x"] * part_params["pins"] + params["fab"]["right"],
            params["fab"]["bottom"] - params["fab"]["top"],
        )
    ...
    body = body.translate(
        (
            (params["pitch"]["x"] * part_params["pins"] + params["fab"]["right"]) / 2
            + params["fab"]["left"],
            -params["fab"]["top"]
            - (params["fab"]["bottom"] - params["fab"]["top"]) / 2,
            0,
...
def generate_pins(params, part_params):
    ...
    # Create a pin
    pin = (
        cq.Workplane("XY", (0, 0, 0)).circle(params["pads"]["drill"] / 2).extrude(-0.5)
    )
    ...
    for i in range(
        0,
        (part_params["pins"] + params["pads"]["increment"] - 1)
        // params["pads"]["increment"],
    ):
        objs.append(
            pin.translate(
                (params["pitch"]["x"] * params["pads"]["increment"] * i, 0, 0)
            )
        )
    for i in range(0, part_params["pins"] // params["pads"]["increment"]):
        objs.append(
            pin.translate(
                (
                    (params["pads"]["increment"] - 1) * params["pitch"]["x"]
                    + params["pitch"]["x"] * params["pads"]["increment"] * i,
                    -params["pitch"]["y"],
                    0,
                )
            )
...
def generate_package3d(params, part_params, mpn):
    ...
    model_name = "PhoenixContact_{series_prefix}{pins}-{orientation_short}-{pitch}{series_sufix}_{rows}x{pins:02d}_P{pitch}_{orientation}_{mpn}".format(
        series_prefix=params["series_prefix"].replace(" ", "_").replace("/", "_"),
        series_sufix=(
            params["series_sufix"] if params["series_sufix"] is not None else ""
        ),
        rows=part_params["rows"],
        pins=part_params["pins"],
        pitch=params["pitch"]["x"],
        orientation=params["orientation"],
        orientation_short=params["orientation"][:1],
        mpn=mpn,
    )
    ...
    add_license.addLicenseToStep(
        output_dir,
        "{model_name}.step".format(model_name=model_name),
        LIST_license,
        LICENCE_Info.STR_licAuthor,
