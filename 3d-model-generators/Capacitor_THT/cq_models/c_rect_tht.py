...
__Comment__ = (
    "make Rect Caps 3D models exported to STEP and VRML for Kicad StepUP script"
)
...
def getName(params, configuration):
    ...
    return format_fn.format(
        length=params.L, width=params.W, pitch=params.F, prefix=prefix, suffix=suffix
    )
...
def generate_part(params):
    ...
    if series == "MKS":
        ...
        pin1 = pin1.translate((-F / 2 + pb / 2, 0, H / 2)).rotate(
            (0, 0, 0), (0, 0, 1), 0
        )
        pin2 = cq.Workplane("XY").box(pb, W, H)
        pin2 = pin2.translate((F / 2 - pb / 2, 0, H / 2)).rotate(
            (0, 0, 0), (0, 0, 1), 0
        )
