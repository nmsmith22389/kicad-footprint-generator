...
def make_models(model_to_build=None, output_dir_prefix=None, enable_vrml=True):
    ...
    for model in models:
        ...
        body = body.translate(all_params[model]['translation']).rotate((0, 0, 0), (0, 0, 1), all_params[model]['rotation'])
        leads = leads.translate(all_params[model]['translation']).rotate((0, 0, 0), (0, 0, 1), all_params[model]['rotation'])
        ...
        if enable_vrml:
            export_VRML(os.path.join(output_dir, model + ".wrl"), [body, leads], [all_params[model]["body_color_key"], all_params[model]["pins_color_key"]])
