...
def generate_part(params):
    ...
    body = (
        cq.Workplane()
        .rect(params["body_width"], params["body_length"])
        .extrude(params["body_thickness"])
    )
    ...
    pins = (
        body.faces(">Z")
        .workplane(offset=params["body_thickness"] / 2.0, invert=True)
        .rarray(
            params["body_width"] + params["pin_thickness"] + 0.001,
            params["body_length"] / 3.0,
            2,
            3,
        )
        .pins_plugin(params)
