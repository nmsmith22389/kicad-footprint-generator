...
def make_models(model_to_build=None, output_dir_prefix=None, enable_vrml=True):
    ...
    for model in models:
        ...
        top_color = shaderColors.named_colors[all_params[model]["top_color_key"]].getDiffuseFloat()
        pin_color = shaderColors.named_colors[all_params[model]["pin_color_key"]].getDiffuseFloat()
        pinmark_color = shaderColors.named_colors[all_params[model]["pinmark_color_key"]].getDiffuseFloat()
        ...
        if not package_found:
            print("Package {} not found, continuing to the next model.".format(all_params[model]['package_type']))
            continue
        ...
        component.add(pinmark, color=cq_color_correct.Color(pinmark_color[0], pinmark_color[1], pinmark_color[2]))
        ...
        if enable_vrml:
            export_VRML(os.path.join(output_dir, file_name + ".wrl"), [body, top, pins, pinmark], [all_params[model]["body_color_key"], all_params[model]["top_color_key"], all_params[model]["pin_color_key"], all_params[model]["pinmark_color_key"]])
