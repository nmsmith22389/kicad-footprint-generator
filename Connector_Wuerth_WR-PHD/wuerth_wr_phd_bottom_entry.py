...
def generate_pinpockets(params, part_params):
    ...
    if params["type"] == "SMD":
        ...
        pocket = (
            ...
            .faces("<Z")
            .rect(1.6, 2)
            .extrude(-params["height"] + 0.2 + 0.3 + 0.5)
        )
        rpocket = cq.Workplane("XY", origin=(0.25, 0) + (0,)).rect(2.1, 2).extrude(0.5)
    ...
    for i in range(0, part_params["pins"] // part_params["rows"]):
        objs.append(
            pocket.translate(
                (
                    params["pitch"] / 2,
                    (params["pitch"] * (part_params["pins"] // 2 - 1)) / 2
                    - i * params["pitch"],
                    0,
                )
            )
...
def generate_body(params, part_params):
    ...
    body = (
        ...
        .rect(
            params["width"],
            2 * params["top"] + params["pitch"] * (part_params["pins"] // 2 - 1),
        )
    ...
    if params["type"] != "SMD":
        body = body.translate(
            (
                params["holes"]["offset"] + params["pitch"] / 2.0,
                -(params["pitch"] * (part_params["pins"] // 2 - 1)) / 2,
                0,
            )
        )
...
def generate_pins(params, part_params):
    ...
    if params["type"] == "SMD":
        ...
        pin = (
            ...
            .center(0, 1.625)
            .rect(0.8, 0.25)
            .extrude(0.6 if params["type"] == "Standard" else 1.87)
        )
        pin = (
            pin.faces(">Z")
            .workplane(centerOption="CenterOfMass")
            .center(-(0.6 if params["type"] == "Standard" else 1.87) / 2, 0)
            .moveTo(0, -0.75)
            .lineTo(-1.15, -0.75)
            .lineTo(-1.15, -1.0)
            .lineTo(0.25, -1.0)
    ...
    for i in range(0, part_params["pins"] // part_params["rows"]):
        objs.append(
            pin.translate(
                (
                    params["pitch"] / 2
                    + (0.35 if params["type"] == "SMD" else params["holes"]["offset"]),
                    (params["pitch"] * (part_params["pins"] // 2 - 1)) / 2
                    - i * params["pitch"],
                    0,
                )
