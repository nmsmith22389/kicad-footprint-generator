...
def _make_pin (pin_index, params, width, thickness):
    ...
    if thickness > pin_height:
        raise ValueError (f"Pin {pin_index + 1}: Thickness can't be more than pin height.")
    ...
    if corner_radius > 0:
        if corner_radius > pin_height:
            raise ValueError (f"Pin {pin_index + 1}: Corner radius can't be bigger then pin height.")
        if corner_radius > pin_length + thickness / 2:
            raise ValueError (f"Pin {pin_index + 1}: Corner radius is too big in relation to pin length.")
