...
calcDim = namedtuple(
    "calcDim", ["length", "cutout_len", "mount_hole_y", "plug_left_side"]
)
...
def dimensions(params):
    ...
    cutout_len = (
        params["num_pins"] * params["pin_pitch"]
        - 1.6
        + (0 if params["pin_pitch"] > 5.08 else 2)
    )
    return calcDim(
        length=length,
        mount_hole_y=2.5 if params["angled"] else 0.0,
        cutout_len=cutout_len,
        plug_left_side=(length - cutout_len) / 2.0 - params["side_to_pin"],
    )
...
def generate_body(params, calc_dim, with_details=False):
    # Lock this to the high detail version
    with_details = True
    ...
    if insert is not None:
        insert = insert.rotate((0, 0, 0), (1, 0, 0), 90)
...
def generate_straight_body(params, calc_dim, with_details):
    ...
    lock_prod = seriesParams.body_lock_protrusion
    ...
    body = (
        cq.Workplane("YZ")
        .workplane(offset=left_side, centerOption="CenterOfMass")
        .moveTo(*start_point)
        .polyline(side_profile, includeCurrent=True)
        .close()
        .extrude(body_len)
    )
    ...
    if with_details:
        plug_cutout = (
            cq.Workplane("XY")
            .workplane(offset=body_height, centerOption="CenterOfMass")
            .moveTo(plug_left_side, -plug_front)
            .rect(plug_len, plug_width, False)
            .extrude(-plug_depth)
        )
    ...
    lock_cutout = (
        cq.Workplane("XZ")
        .workplane(offset=-body_width + front_side, centerOption="CenterOfMass")
        .moveTo(-lock_cutout_t_l / 2.0, body_height)
        .hLine(lock_cutout_t_l)
        .line(-(lock_cutout_t_l - lock_cutout_b_l) / 2.0, -lock_chamf_h)
        .hLine(-lock_cutout_b_l)
        .close()
        .extrude(back_width)
    )
...
def generate_plug_straight(params, calc_dim):
    ...
    plug_body = (
        cq.Workplane("YZ")
        .workplane(offset=-psv_to_0, centerOption="CenterOfMass")
        .moveTo(*psvp[0])
        .lineTo(*psvp[1])
        .lineTo(*psvp[2])
        .lineTo(*psvp[3])
    ...
    if not params["flanged"]:
        ...
        plug_lock = (
            cq.Workplane("YZ")
            .workplane(offset=pl_wp_offset, centerOption="CenterOfMass")
            .moveTo(*plp[0])
            .lineTo(*plp[1])
        ...
        plug_lock = plug_lock.edges(
            BS(
                ...
                (pl_wp_offset - plug_lock_width + 0.1, 11, seriesParams.body_height),
            )
        ).fillet(0.3)

        plug_body = plug_body.union(plug_lock)
        if params["num_pins"] > 2:
            plug_body = plug_body.union(
                plug_lock.translate(
                    (
                        (params["num_pins"] - 2) * params["pin_pitch"]
                        + plug_lock_width,
                        0,
                        0,
                    )
    ...
    for i in range(params["num_pins"]):
        plug_body = plug_body.cut(
            single_screw_cutout.translate((i * params["pin_pitch"], 0, 0))
        )
    ...
    if params["flanged"]:
        plug_flange = (
            ...
            .moveTo(calc_dim.length / 2 - params["side_to_pin"])
            .rect(calc_dim.length, 5.8)
            .extrude(7.7)
        )  # \
        # .edges("|Z").fillet(1)
        mh_distance = (
        ...
        plug_mount_screw = (
            plug_flange.faces(">Z")
            .workplane(offset=-6, centerOption="CenterOfMass")
            .moveTo(mh_distance / 2.0)
            .circle(1.95)
            .extrude(2, False)
            .faces(">Z")
        ...
        p1 = (x1, -5.8 / 2 - 0.01, seriesParams.body_height + 1)
        p2 = (
            x1 + calc_dim.length + 0.2,
            5.8 / 2 + 0.01,
    ...
    wire_cutout = (
        plug_body.faces(">Z")
        .workplane(centerOption="CenterOfMass")
        .moveTo(-(params["num_pins"] - 1) * params["pin_pitch"] / 2.0, 0.8)
        .rect(3.5, 6.7)
        .workplane(offset=-2, centerOption="CenterOfMass")
        .moveTo(0, 0.9)
        .rect(3, 3.2)
...
def generate_plug(params, calc_dim):
    ...
    plug = plug.rotate((0, 0, 0), (1, 0, 0), 90)
    plug = plug.translate((0, front_side + pin_angled_from_back, 0))
    plug_screws = plug_screws.rotate((0, 0, 0), (1, 0, 0), 90)
    plug_screws = plug_screws.translate((0, front_side + pin_angled_from_back, 0))

    return plug, plug_screws


def generate_part(series_params, with_plug=False):
    # params = all_params[part_key]
    calc_dim = dimensions(series_params)
    pins = generate_pins(series_params)
    body, insert = generate_body(series_params, calc_dim, not with_plug)
    mount_screw = generate_mount_screw(series_params, calc_dim)

    plug, plug_screws = (None, None)
    if with_plug:
        plug, plug_screws = generate_plug(series_params, calc_dim)
