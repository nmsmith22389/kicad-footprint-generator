...
__Comment__ = (
    "make C axial Caps 3D models exported to STEP and VRML for Kicad StepUP script"
)
...
def getName(params, configuration):
    ...
    return format_fn.format(
        width=params.W, diameter=params.L, pitch=params.F, prefix=prefix, suffix=suffix
    )
...
def generate_part(params):
    ...
    lead1 = (
        ...
        .circle(d / 2)
        .sweep(path)
        .translate((-params["F"] / 2.0, 0, -W / 2.0))
    )
    leads = lead1
    ...
    body = (
        cq.Workplane("XY")
        .workplane(offset=L / 2 + bs, centerOption="CenterOfMass")
        .moveTo(0, W / 2)
        .threePointArc((-L / 4, point2), (-L / 2, 0), forConstruction=False)
