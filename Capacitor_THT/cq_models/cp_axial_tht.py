...
__Comment__ = (
    "make Radial Caps 3D models exported to STEP and VRML for Kicad StepUP script"
)
...
from math import radians, sin, sqrt, tan
...
class series_params:
    ...
    obj_suffixes = ["__body", "__mark_vg", "__mark_bg", "__pins", "__endcaps"]
...
def generate_part(params):
    ...
    ciba = 45.0  # angle of the cathode identification bar
    ...
    def bodyp(off=0):
        ...
        return (
            ...
            .hLineTo(D / 2.0 - ef, 0)
            .threePointArc(
                (D / 2.0 - (ef_x) + tpa_off, bs + (ef_x) - tpa_off),
                (D / 2.0 + off, bs + ef),
            )
            .line(0, bt)
            .threePointArc(
                (D / 2.0 - bd + off, bs + bt + bh / 2.0), (D / 2.0 + off, bs + bt + bh)
            )
            .lineTo(D / 2.0 + off, L + bs - ef)
            .threePointArc(
                (D / 2.0 - (ef_x) + tpa_off, L + bs - (ef_x) + tpa_off),
                (D / 2.0 - ef, L + bs + off),
            )
            .hLineTo(dc / 2)
            .vLine(-(tc + tmt))
    ...
    bottom = cq.Workplane("XY").workplane(offset=bs + tc).circle(dc / 2).extrude(bpt)
    # draw the metallic part at the top
    top = (
        cq.Workplane("XY")
        .workplane(offset=bs + L - tc - ts)
        .circle(dc / 2)
        .extrude(tmt)
    )
    ...
    # draw end knobs
    topknob = (
        cq.Workplane("XY").workplane(offset=bs + tc).circle(D / 8).extrude(-D / 30)
    )
    bottomknob = (
        cq.Workplane("XY")
        .workplane(offset=bs + L - tc - ts + tmt)
        .circle(D / 8)
        .extrude(D / 30)
    )
