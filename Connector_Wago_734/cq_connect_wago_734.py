...
class cqMakerWagoConn734:
    ...
    def __init__(self, parameter):
        ...
        self.body_lenght = (
            self.num_pins - 1
        ) * self.pin_pitch + 5.9  # formula from data sheet
        ...
        self.body_offset_y = (
            (self.body_width / 2.0 - self.pin_to_body_front) if self.isVertical else 0.0
        )  # the y-shift of horizontal connectors will be done after 90 deg rotation
        self.body_offset = (self.body_offset_x, self.body_offset_y, 0.0)
        self.body_guide_notch_width = 2.0  # inner guide notches
        self.body_side_pocket_width = (
            5.4  # outer pocket at left and right side of housing
        )
        ...
        self.pin_pockets_width = (
            2.8  # pockets at pin positions at the back, front and bottom of housing
        )
        self.pin_length_inside_body = (
            self.pin_length - self.pin_length_below_body if self.isVertical else 8.2
        )
    ...
    def __innerPocket(self):
        ...
        body = (
            cq.Workplane("XY")
            .workplane(centerOption="CenterOfMass", offset=self.body_thickness_z)
            .rect(
                self.body_lenght - 2.0 * self.body_thickness_x,
                self.body_width - 2.0 * self.body_thickness_y,
            )
        ...
        body = body.union(
            body_guide_notch.translate(
                (
                    (self.body_lenght - l) / 2.0 - self.body_thickness_x,
                    -((self.body_width + w) / 2.0 - self.body_thickness_y),
                    1.8,
                )
            )
        )
        ...
        body_guide_notch = (
            cq.Workplane("XY")
            .workplane(centerOption="CenterOfMass", offset=self.body_thickness_z)
            .rect(self.body_lenght - 2.0 * self.body_thickness_x + 0.8, l)
            .extrude(self.body_height)
        )
    ...
    def __outerPocketLeft(self):
        ...
        body_i = (
            cq.Workplane("YZ")
            .workplane(centerOption="CenterOfMass", offset=-self.body_lenght / 2.0)
            .rect(
                self.body_guide_notch_width,
                2.0 * (self.body_height - self.body_thickness_y),
            )
            .extrude(self.body_thickness_x)
        )

        body_o = (
            cq.Workplane("YZ")
            .workplane(
                centerOption="CenterOfMass", offset=-self.body_lenght / 2.0 + 0.4
            )
            .rect(
                self.body_side_pocket_width,
                2.0 * (self.body_height - self.body_thickness_y),
    ...
    def makePlasticCase(self):
        ...
        pin_pad = (
            cq.Workplane("XY")
            .workplane(centerOption="CenterOfMass", offset=self.body_thickness_z)
            .rect(self.pin_pockets_width, self.pin_pockets_width)
            .extrude(1.0)
        )
        ...
        pin_pocket_back = (
            cq.Workplane("XZ")
            .rect(
                self.pin_pockets_width, self.body_height * 2.0 - self.body_thickness_y
            )
            .extrude(self.body_thickness_y)
        )

        allPockets = self._duplicatePins(
            pin_pocket_back.translate(self.first_pin_pos).translate(
                (0.0, (self.body_height - self.body_thickness_y) / 2.0 + 0.4, 0.0)
            )
        )
        ...
        pin_pocket_front_a = (
            cq.Workplane("XZ")
            .rect(
                self.pin_pockets_width / 2.0,
                self.body_height * 2.0 - self.body_thickness_y,
            )
            .extrude(-self.body_thickness_y)
        )
        pin_pocket_front_b = (
        ...
        allPockets = self._duplicatePins(
            pin_pocket_front.translate(self.first_pin_pos).translate(
                (0.0, -(self.body_height - self.body_thickness_y) / 2.0 - 0.6, 0.0)
            )
        )
        ...
        if self.num_pins >= 3:
            ...
            body_cut = (
                cq.Workplane("XY")
                .workplane(centerOption="CenterOfMass", offset=self.body_height / 2.0)
                .rect(self.pin_pockets_width, self.body_width * 2.0)
                .extrude(self.body_height)
        ...
        allPockets = self._duplicatePins(
            pin_pocket_bottom.translate(self.first_pin_pos)
        )
        body = body.cut(allPockets)

        # if connector is horizontal it must be rotated and shifted
        if not self.isVertical:
            body = body.rotate((0, 0, 0), (1, 0, 0), 90.0).translate(
                (0.0, self.body_height - self.pin_to_body_front, self.body_width / 2.0)
            )
    ...
    def _make_straight_pin(self):
        ...
        body = (
            cq.Workplane("XY")
            .rect(self.pin_width, self.pin_thickness)
            .extrude(self.pin_length)
            .edges("<Z")
            .chamfer(1.0, self.pin_width / 4.0)
            .edges(">Z")
            .chamfer(self.pin_width / 4.0)
    ...
    def _make_angled_pin(self):
        ...
        path = (
            cq.Workplane("YZ")
            .lineTo(0.0, self.pin_length - r_mean)
    ...
    def make_pins(self):
        ...
        firstPin = (
            self._make_straight_pin() if self.isVertical else self._make_angled_pin()
        )
...
class partConnWago734:
    ...
    def _createPartName(self, num_pin, ori):
        ...
        return "Wago_734-{0}_1x{1:02d}_P3.50mm_{2}".format(
            serie + num_pin, num_pin, ori
        )
        ...
        for ori in ["Vertical", "Horizontal"]:
            for i in range(2, 25):
                if i not in invalidPinNumber:
                    ...
                    self.all_params[name] = self.Params(
                        modelName=name, num_pins=i, ori=ori
                    )
