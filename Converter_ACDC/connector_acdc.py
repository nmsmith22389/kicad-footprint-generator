...
def make_case(params):
    ...
    if rim != None:
        if len(rim) == 3:
            ...
            print("rdh " + str(rdh) + '\r\n')
            print('\r\n')
            case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=A1).moveTo(mvX + rdx, mvY).rect(L - (2.0 * rdx), 0 - (W + 1.0), False).extrude(rdh)
            case = case.cut(case1)
            case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=A1).moveTo(mvX, mvY - rdy).rect(L + 1.0, 0 - (W - (2.0 * rdy)), False).extrude(rdh)
            case = case.cut(case1)
...
def make_case_top(params):
    ...
    if show_top == 1:
        ...
        if (pintype == CASE_SMD_TYPE):
            ...
            casetop=cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=tty).moveTo(mvX, mvY).rect(L1, W1, False).extrude(0.2)
...
def make_pins_tht_n(params):
    ...
    for i in range(1, len(pin)):
        p = pin[i]
        pint=cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=A1 + 2.0).moveTo(p[0], -p[1]).circle(pinpadsize / 2.0, False).extrude(0 - (pinpadh + 2.0))
        pint = pint.faces("<Z").fillet(pinpadsize / 5.0)
        pins = pins.union(pint)
        pint=cq.Workplane("XZ").workplane(centerOption="CenterOfMass", offset= 0 -p[1]).moveTo(p[0], 2.0).circle(pinpadsize / 2.0, False).extrude( 0 - (W / 2.0))
        pins = pins.union(pint)
    ...
    for i in range(0, len(pin)):
        ...
        if p[0] < 0 and (p[1] > (0 - (W / 2.0)) and p[1] < ((W / 2.0))):
            ...
            if p[0] < (0 - (L / 2.0)):
                ...
                xD = p[0]
                yD = pinpadsize
                pint=cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0).moveTo(myX1, myY1).rect(xD, yD).extrude(pinpadh)
            else:
        ...
        #
            ...
            if p[0] > (L / 2.0):
                ...
                xD = pinpadsize
                yD = pinpadsize
                pint=cq.Workplane("ZY").workplane(centerOption="CenterOfMass", offset=0 - ((L / 2.0) + (pinpadh / 2.0))).moveTo(myZ1, myY1).rect(xD, yD).extrude(pinpadh)
        elif p[1] < 0:
            # top pad
            if p[1] < (W / 2.0):
                ...
                xD = pinpadsize
                myX1 = p[0] - (pinpadsize / 2.0)
                pint=cq.Workplane("ZX").workplane(centerOption="CenterOfMass", offset=((W / 2.0) - (pinpadh / 2.0))).moveTo(myZ1, myX1).rect(xD, yD).extrude(pinpadh)
        else:
            # bottom pad
            if p[1] > (W / 2.0):
                myX1 = p[0] - (pinpadsize / 2.0)
                ...
                yD = pinpadsize
                xD = pinpadsize
                pint=cq.Workplane("ZX").workplane(centerOption="CenterOfMass", offset=0 -((W / 2.0) + (pinpadh / 2.0))).moveTo(myZ1, myX1).rect(xD, yD).extrude(pinpadh)
