...
class dip_socket_turned_pin(PartBase):
    ...
    def __init__(self, params):
        ...
        self.body_bottom_pockets = (
            0
            if self.num_pins <= 4
            else (1 if self.num_pins <= 10 else (3 if self.num_pins > 28 else 2))
        )
    ...
    def _make_pinpockets(self):
        ...
        # create first pocket
        pocket = (
            cq.Workplane("XY", origin=(self.first_pin_pos + (0.0,)))
    ...
    def _make_pin_socket(self):
        pin = (
            cq.Workplane("XY")
            .circle(self.pin_socket_radius)
            .extrude(-self.body_board_distance)
            .faces("<Z")
            .edges()
            .chamfer(0.3)
        )
        ...
        return (
            ...
            .circle(self.pin_socket_radius + 0.1)
            .extrude(self.body_height + 0.04)
            .faces(">Z")
            .cskHole(
                self.pin_radius * 2.0,
                self.pin_socket_diameter,
                82,
    ...
    def _make_pin_smd(self):
        ...
        self.offsets = (
            self.offsets[0],
            self.offsets[1],
            self.offsets[2] - s + self.pin_radius,
        )
