...
class dip_switch_piano(PartBase):
    ...
    def __init__(self, params):
        ...
        self.offsets = (
            (self.body_length / 2.0) - 1.40 - self.pin_width,
    ...
    def _make_switchpockets(self):
        ...
        pocket = (
            cq.Workplane("XZ", origin=(self.first_pin_pos[0], 0.0, 8.0))
        ...
        pocket2 = (
            cq.Workplane("YZ")
            .workplane(
                centerOption="CenterOfMass", offset=-self.body_length / 2.0 + 1.0
            )
    ...
    def make_body(self):
        ...
        body = body.edges(
            BS((bl + 0.1, -bw - 0.1, bz - 0.1), (-bl - 0.1, -bw + 0.1, bz + 0.1))
        ).chamfer(2.0)
        body = body.edges(
            BS((bl + 0.1, bw - 0.1, bz - 0.1), (-bl - 0.1, bw + 0.1, bz + 0.1))
        ).fillet(1.0)
    ...
    def make_buttons(self):
        ...
        button = (
            cq.Workplane(
                "XZ", origin=(self.first_pin_pos[0], self.body_width / 2.0 - 3.3, 8.0)
            )
            .rect(self.button_width, self.button_base)
            .extrude(0.7)
            .faces(">Y")
            .center(0, self.button_base / 2 - self.button_length / 2.0)
...
class dip_switch_piano_cts(dip_switch_piano):
    ...
    def __init__(self, params):
        ...
        self.offsets = (
            (self.body_length / 2.0) - 1.85 - self.pin_width,
            self.pin_rows_distance / 2.0,
            self.body_board_distance,
        )
