...
class Ribbon:
    ...
    def drawRibbon(self, debug=False):
        if self.commands[0][0] == "start":
            ...
            self.current_x = self.commands[0][1]["position"][0] + half_width * np.cos(
                np.deg2rad(self.direction + 90)
            )
            self.current_y = self.commands[0][1]["position"][1] + half_width * np.sin(
                np.deg2rad(self.direction + 90)
            )
