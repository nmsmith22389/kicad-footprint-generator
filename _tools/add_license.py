...
LIST_int_license = [
    "Copyright (C) " + datetime.now().strftime("%Y") + ", " + STR_int_licAuthor,
    "",
...
def FNCT_modify_step(
    ...
    for line in range(DICT_positions["E"]):
        ...
        if line >= DICT_positions["D"] and line < (
            DICT_positions["N"] - 1
        ):  # get DESCR part
        ...
        if line == (DICT_positions["S"] - 1):  # add modded DESCR and NAME
            ...
            PMBL_modstepfile.append(
                "/* description */ ('model of " + str(fname) + "'),"
            )
            STR_description = (
                pyparsing.nestedExpr("/*", "*/")
                .suppress()
                .transformString(STR_description)
            )
...
def addLicenseToStep(
    ...
    if os.path.isfile(filepath):  # test if folder or file..
        ...
        LIST_license[0] = (
            "Copyright (C) " + datetime.now().strftime("%Y") + ", " + STR_licAuthor
        )
        if ext == ".stp" or ext == ".step":  # test if step file
            ...
            LIST_PMBL = FNCT_modify_step(
                PMBL_stepfile[: DICT_positions["E"]],
                DICT_positions,
                LIST_license,
                FNME_stepfile,
                STR_licAuthor,
            ...
            print(
                "Add License: File has wrong file extension: {:s}".format(FNME_stepfile)
            )
    else:
