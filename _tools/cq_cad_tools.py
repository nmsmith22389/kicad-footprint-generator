...
__author__ = "maurice"
__Comment__ = (
    "CadQuery exporting and fusion libs to generate STEP and VRML models with colors"
)
    ...
    if "-" in string:
        ...
        try:
            ...
            FreeCAD.Console.PrintWarning(
                "Error in getListOfNumbers single number case:"
            )
            FreeCAD.Console.PrintWarning("{:s}\n".format(exp))
...
def close_CQ_Example(App, Gui):
    ...
    for widget in dockWidgets:
        if (
            (widget.objectName() == "Report view")
            or (widget.objectName() == "Python console")
            or (widget.objectName() == "Combo View")
        ):
...
###################################################################
def FuseObjs_wColors(App, Gui, docName, part1, part2, keepOriginals=False):
    ...
    App.activeDocument().Fusion.Shapes = [
        App.activeDocument().getObject(part1),
        App.activeDocument().getObject(part2),
    ]
    Gui.activeDocument().getObject(part1).Visibility = False
    Gui.activeDocument().getObject(part2).Visibility = False
    Gui.ActiveDocument.Fusion.ShapeColor = (
        Gui.activeDocument().getObject(part1).ShapeColor
    )
    Gui.ActiveDocument.Fusion.DisplayMode = (
        Gui.activeDocument().getObject(part1).DisplayMode
    )
...
def SimpleCopy_wColors(obj):
    ...
    FreeCADGui.ActiveDocument.ActiveObject.ShapeColor = (
        FreeCADGui.ActiveDocument.getObject(obj.Name).ShapeColor
    )
    FreeCADGui.ActiveDocument.ActiveObject.LineColor = (
        FreeCADGui.ActiveDocument.getObject(obj.Name).LineColor
    )
    FreeCADGui.ActiveDocument.ActiveObject.PointColor = (
        FreeCADGui.ActiveDocument.getObject(obj.Name).PointColor
    )
    FreeCADGui.ActiveDocument.ActiveObject.DiffuseColor = (
        FreeCADGui.ActiveDocument.getObject(obj.Name).DiffuseColor
    )
    FreeCADGui.ActiveDocument.ActiveObject.Transparency = (
        FreeCADGui.ActiveDocument.getObject(obj.Name).Transparency
    )
...
def multiFuseObjs_wColors(App, Gui, docName, objs, keepOriginals=False):
    ...
    Gui.ActiveDocument.Fusion.ShapeColor = Gui.ActiveDocument.getObject(
        objs[0].Name
    ).ShapeColor
    Gui.ActiveDocument.Fusion.DisplayMode = Gui.ActiveDocument.getObject(
        objs[0].Name
    ).DisplayMode
    App.ActiveDocument.recompute()

    App.ActiveDocument.addObject("Part::Feature", "Fusion").Shape = (
        App.ActiveDocument.Fusion.Shape
    )
    App.ActiveDocument.ActiveObject.Label = docName
    fused_obj = App.ActiveDocument.ActiveObject
    ...
    Gui.ActiveDocument.ActiveObject.LineColor = Gui.ActiveDocument.Fusion.LineColor
    Gui.ActiveDocument.ActiveObject.PointColor = Gui.ActiveDocument.Fusion.PointColor
    Gui.ActiveDocument.ActiveObject.DiffuseColor = (
        Gui.ActiveDocument.Fusion.DiffuseColor
    )
    App.ActiveDocument.recompute()
...
#   Function to fuse two objects together.
###################################################################
def FuseObjs_wColors_naming(App, Gui, docName, part1, part2, name, keepOriginals=False):
...
#   Function to fuse two objects together.
###################################################################
def CutObjs_wColors(App, Gui, docName, part1, part2):
...
def z_RotateObject(doc, rot):
    ...
    FreeCAD.getDocument(doc.Name).getObject(objs[0].Name).Placement = FreeCAD.Placement(
        FreeCAD.Vector(0, 0, 0), FreeCAD.Rotation(FreeCAD.Vector(0, 0, 1), rot)
    )
...
def exportVRML(doc, modelName, scale, dir):
    ...
    FreeCAD.ActiveDocument.addObject("Part::Feature", "Vrml_model").Shape = objs[
        0
    ].Shape
    FreeCAD.ActiveDocument.ActiveObject.Label = "Vrml_model"
    FreeCADGui.ActiveDocument.ActiveObject.ShapeColor = (
        FreeCADGui.getDocument(doc.Name).getObject(objs[0].Name).ShapeColor
    )
    FreeCADGui.ActiveDocument.ActiveObject.LineColor = (
        FreeCADGui.getDocument(doc.Name).getObject(objs[0].Name).LineColor
    )
    FreeCADGui.ActiveDocument.ActiveObject.PointColor = (
        FreeCADGui.getDocument(doc.Name).getObject(objs[0].Name).PointColor
    )
    FreeCADGui.ActiveDocument.ActiveObject.DiffuseColor = (
        FreeCADGui.getDocument(doc.Name).getObject(objs[0].Name).DiffuseColor
    )
    FreeCAD.ActiveDocument.recompute()
    ...
    FreeCAD.Console.PrintMessage(
        FreeCAD.ActiveDocument.ActiveObject.Label + " exported and scaled to Vrml\r\n"
    )
    del __objs__
...
def saveFCdoc(App, Gui, doc, modelName, dir, saving=True):
    ...
    try:
        ...
        FreeCAD.Console.PrintWarning(
            "Error while trying to remove backup file in saveFCdoc:"
        )
        FreeCAD.Console.PrintWarning("{:s}\n".format(str(exp)))
    if saving == False:
        try:
            ...
            FreeCAD.Console.PrintWarning(
                "Error while trying to remove file in saveFCdoc (with save == False):"
            )
            FreeCAD.Console.PrintWarning("{:s}\n".format(exp))
    else:
...
def checkRequirements(cq):
    ...
    FC_majorV = str(FreeCAD.Version()[0]).replace(",", ".")
    FC_minorV = str(FreeCAD.Version()[1]).replace(",", ".")
    ...
    if int(float(FC_majorV)) <= 0:
        if int(float(FC_minorV)) < 15:
            reply = QtGui.QMessageBox.information(
                None,
                "Warning! ...",
                "use FreeCAD version >= " + FC_majorV + "." + FC_minorV + "\r\n",
            )
...
#
###################################################################
def runGeometryCheck(
    App, Gui, step_path, log, modelName, save_memory=True, stop_on_first_error=True
):
