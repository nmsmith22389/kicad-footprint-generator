...
class cq_coaxial_amphenol():
    ...
    def make_top_SMA_Amphenol_132134(self, params):
        ...
        case = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=AZ1).moveTo(AX1, AY1).circle((AW1 / 2.0) - 1.0, False).extrude(AL1)
        #
        # Cut out center pin
        #
    ...
    def make_case_SMA_Amphenol_132134(self, params):
        ...
        # cut out under body
        if BT > 0.001:
            case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0.0).moveTo(0.0, 0.0).rect(W / 2.0, 2.0 * L).extrude(BT)
            case = case.cut(case1)
            case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0.0).moveTo(0.0, 0.0).rect(2.0 * L, W / 2.0).extrude(BT)
            case = case.cut(case1)
        
        # Base
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=AZ).moveTo(0.0, 0.0).circle((AW / 2.0), False).extrude(AL)
        case = case.union(case1)
        # Top cylinder
        case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=AZ1).moveTo(0.0, 0.0).circle((AW1 / 2.0) - 0.2, False).extrude(AL1)
        # Add the taps
        tt = AZ1 + 0.4
        while tt < (AZ1 + AL1 - 0.6):
#        while ty < (AY1 + 2.0):
            case2 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=0.0).moveTo(0.0, 0.0).circle(AW1 / 1.8, False).extrude(0.2)
        ...
        # Cut out hollow
        case2 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=AZ1 + 0.2).moveTo(0.0, 0.0).circle((AW1 / 2.0) - 0.4, False).extrude(AL1 + 2.0)
        case1 = case1.cut(case2)
        # Center pin
        case2 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=AZ1).moveTo(AX1, AY1).circle(0.8, False).extrude(AL1)
        case3 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=AZ1).moveTo(AX1, AY1).circle(0.76, False).extrude(AL1)
    ...
    def make_pin(self, params):
        ...
        if pin == None:
            case = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=1.0).moveTo(0.0, 0.0).circle(0.01, False).extrude(0.01)
            ...
            for n in pin:
                ...
                ld = n[5]
                if t == 'round':
                    case1 = cq.Workplane("XY").workplane(centerOption="CenterOfMass", offset=1.0).moveTo(0.0, 0.0).circle(w / 2.0, False).extrude(0.0 - (ld + 1.0))
