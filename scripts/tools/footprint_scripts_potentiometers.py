...
def makePotentiometerHorizontal(class_name="", wbody=0, hbody=0, dscrew=0, style="normal", ddrill=0,
    ...
    # add model
    if (has3d != 0):
        kicad_modg.append(Model(filename="${KICAD8_3DMODEL_DIR}/" + lib_name + "_THT.3dshapes/" + footprint_name + ".wrl", at=x_3d, scale=s_3d, rotate=r_3d))
...
def makePotentiometerVertical(class_name, wbody, hbody, screwstyle="none", style="normal", d_body=0, dshaft=6,
    ...
    if (has3d != 0):
        if SMD_pads:
            kicad_modg.append(Model(filename="${KICAD8_3DMODEL_DIR}/" + lib_name + "_SMD.3dshapes/" + footprint_name + ".wrl", at=x_3d, scale=s_3d, rotate=r_3d))
        else:
            kicad_modg.append(Model(filename="${KICAD8_3DMODEL_DIR}/" + lib_name + "_THT.3dshapes/" + footprint_name + ".wrl", at=x_3d, scale=s_3d, rotate=r_3d))
