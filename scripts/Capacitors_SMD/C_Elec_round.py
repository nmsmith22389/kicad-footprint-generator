...
def create_footprint(name, configuration, **kwargs):
    ...
    if ('lead_length' in kwargs) and ('lead_width' in kwargs) and ('lead_spacing' in kwargs):
        ...
        device_dimensions['lead_outside'] = TolerancedSize(maximum=device_dimensions['lead_spacing'].maximum +
    ...
    kicad_mod.append(Line(start=[-fab_x, fab_y_edge], end=[-fab_x_edge, fab_y],
                          layer='F.Fab', width=configuration['fab_line_width']))
    kicad_mod.append(Circle(center=[0, 0], radius=body_size['diameter']/2.0,
                            layer='F.Fab', width=configuration['fab_line_width']))
    ...
    silk_y_start = pad_params['size'][1] / 2.0 + \
        configuration['silk_pad_clearance'] + configuration['silk_line_width']/2.0
...
if __name__ == "__main__":
    ...
    parser.add_argument('--series_config', type=str, nargs='?', help='the config file defining series parameters.',
                        default='../SMD_chip_package_rlc-etc/config_KLCv3.0.yaml')
    parser.add_argument('--ipc_definition', type=str, nargs='?',
                        help='the IPC definition file', default='ipc7351B_capae_crystal.yaml')
