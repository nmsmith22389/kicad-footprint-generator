...
class CrystalResonatorOscillatorGenerator(FootprintGenerator):
    ...
    def makeSMDCrystalAndHand(self, footprint_name, addSizeFootprintName, pins, pad_sep_x, pad_sep_y, pad, pack_width,
        ...
        self.makeSMDCrystal(footprint_name, addSizeFootprintName, pins, pad_sep_x, pad_sep_y, pad, pack_width,
        ...
        self.makeSMDCrystal(footprint_name, addSizeFootprintName, pins, pad_sep_x + pad[0] * (hsfactorx - 1),
    ...
    def makeSMDCrystal(self, footprint_name, addSizeFootprintName, pins, pad_sep_x, pad_sep_y, pad, pack_width, pack_height,
                    pack_bevel, hasAdhesive=False, adhesivePos=[0, 0], adhesiveSize=1, style="rect",
                    description="Crystal SMD SMT", tags=[], lib_name="Crystal", offset3d=[0, 0, 0], scale3d=[1, 1, 1],
        ...
        desc = description + ", {0:2.1f}x{1:2.1f}mm^2 package, generated by make_crystal.py".format(pack_width, pack_height)
        tag_s = tags
        ...
        kicad_modg.append(
            Property(name=Property.REFERENCE,
                     ...
                     layer='F.SilkS'))
        kicad_modg.append(Text(type='user', text='${REFERENCE}', at=[0, 0], layer='F.Fab'))
        kicad_modg.append(
        ...
        if (pins == 2):
            kicad_modg.append(Pad(number=1, type=pad_type, shape=pad_shape1, at=[-pad_sep_x / 2, 0], size=pad, drill=ddrill,
                                layers=[pad_layers + '.Cu', pad_layers + '.Mask', pad_layers + '.Paste']))
        ...
        # add model
        kicad_modg.append(
            Model(filename=self.get_standard_3d_model_path(lib_name, modelname), at=offset3d, scale=scale3d, rotate=rotate3d)
        )
    ...
    def makeCrystalAll(self, footprint_name, rm, pad_size, ddrill, pack_width, pack_height, pack_offset, pack_rm, style="flat",
                    ...
                    lib_name="Crystal", tags="", offset3d=[0, 0, 0], scale3d=[1, 1, 1], rotate3d=[0, 0, 0],
                    name_addition="", pad_style="tht", script3d="", height3d=4.65, iheight3d=4):
        self.makeCrystal(footprint_name, rm, pad_size, ddrill, pack_width, pack_height, pack_offset, pack_rm, style,
    ...
    def makeCrystal(self, footprint_name, rm, pad_size, ddrill, pack_width, pack_height, pack_offset, pack_rm, style="flat",
                    package_pad=False, package_pad_add_holes=False, package_pad_offset=0, package_pad_size=[0, 0],
                    package_pad_drill_size=[1.2, 1.2], package_pad_ddrill=0.8, description="Crystal THT",
                    lib_name="Crystal", tags="", offset3d=[0, 0, 0], scale3d=[1, 1, 1], rotate3d=[0, 0, 0],
                    name_addition="", pad_style="tht", script3d="", height3d=4.65, iheight3d=4):
        ...
        if package_pad:
            if (package_pad_size[1] < pack_width / 2):
                h_slk=math.fabs((pad3pos[1]-package_pad_size[0]/2-slk_offset)-t_slk) + 2 * slk_clearance
                slk_u_line = True
            else:
                h_slk = max(t_slk + h_slk, pad3pos[1] + package_pad_size[0] / 2 + slk_offset + slk_clearance) - t_slk
            l_slk = min(l_slk, rm / 2 - package_pad_size[1] / 2 - slk_offset - slk_clearance)
            w_slk = max(l_slk+w_slk, rm / 2 + package_pad_size[1] / 2 + slk_offset + slk_clearance)-l_slk
...
#                myfile.write("print(\"created {0}\")\n".format(fpname))
        ...
        kicad_modg.append(
            Property(name=Property.REFERENCE, text='REF**',
                     ...
                     layer='F.SilkS',
                     rotation=90))
        kicad_modg.append(Text(type='user', text='${REFERENCE}',
        ...
        # create SILKSCREEN-layer
        if package_pad and package_pad_add_holes:
            kicad_modg.append(PolygonLine(polygon=[[l_slk, pad3pos[1] - package_pad_drill_size[1] / 2 - slk_offset - slk_clearance],
                                                    [l_slk, t_slk],
                                                    [l_slk + w_slk, t_slk],
                                                    [l_slk + w_slk,
                                                    pad3pos[1] - package_pad_drill_size[1] / 2 - slk_offset - slk_clearance]],
                                        layer='F.SilkS', width=lw_slk))
        else:
            if slk_u_line:
        ...
        pad_layers = '*'
        if pad_style == "smd":
            kicad_modg.append(Pad(number=1, type=Pad.TYPE_SMT, shape=Pad.SHAPE_ROUNDRECT, at=[0, 0], size=pad, drill=0,
                                layers=['F.Cu', 'F.Mask', 'F.Paste']))
            kicad_modg.append(Pad(number=2, type=Pad.TYPE_SMT, shape=Pad.SHAPE_ROUNDRECT, at=[rm, 0], size=pad, drill=0,
                                layers=['F.Cu', 'F.Mask', 'F.Paste']))
        else:
        ...
        if package_pad:
            kicad_modg.append(Pad(number=3, type=Pad.TYPE_SMT, shape=Pad.SHAPE_ROUNDRECT, at=pad3pos,
                                size=[package_pad_size[1], package_pad_size[0]], drill=0, layers=['F.Cu', 'F.Mask', 'F.Paste']))
            if package_pad_add_holes:
                kicad_modg.append(
        ...
        # add model
        kicad_modg.append(
            Model(filename=self.get_standard_3d_model_path(lib_name, fpname), at=offset3d, scale=scale3d, rotate=rotate3d))
    ...
    def makeCrystalHC49Vert(self, footprint_name, pins, rm, pad_size, ddrill, pack_width, pack_height, innerpack_width, innerpack_height,
                    description="Crystal THT", lib_name="Crystal", tags="", offset3d=[0, 0, 0], scale3d=[1, 1, 1], rotate3d=[0, 0, 0], addSizeFootprintName=False,
...
#                myfile.write("print(\"created {0}\")\n".format(fpname))
    ...
    def makeCrystalRoundVert(self, footprint_name, rm, pad_size, ddrill, pack_diameter,
                            description="Crystal THT", lib_name="Crystal", tags="", offset3d=[0, 0, 0], scale3d=[1, 1, 1],
                            rotate3d=[0, 0, 0]):
        fpname = footprint_name
