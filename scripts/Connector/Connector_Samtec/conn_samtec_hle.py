...
def generate_one_footprint(global_config: GC.GlobalConfig, pins_per_row, variant, configuration):
    ...
    kicad_mod.append(PolygonLine(shape=poly_f,
    ...
    kicad_mod.append(PolygonLine(shape=p1m_poly,
                                 width=configuration['fab_line_width'], layer="F.Fab"))
    ...
    if ((pad_row1_y - pad_size[1]/2 - s_pad_offset) < body_edge['top']) and ((pad_row1_y + pad_size[1]/2 + s_pad_offset) > body_edge['top']):
        ...
        kicad_mod.append(PolygonLine(shape=poly_s_l,
        ...
        kicad_mod.append(PolygonLine(shape=poly_s_r,
                                     width=configuration['silk_line_width'], layer="F.SilkS"))
        ...
        kicad_mod.append(PolygonLine(shape=poly_s,
                                     width=configuration['silk_line_width'], layer="F.SilkS"))
    ...
    cy_top = round_to_grid(bounding_box['top'] - cy_offset, cy_grid)
    cy_bottom = round_to_grid(bounding_box['bottom'] + cy_offset, cy_grid)
    cy_left = round_to_grid(bounding_box['left'] - cy_offset, cy_grid)
    cy_right = round_to_grid(bounding_box['right'] + cy_offset, cy_grid)
