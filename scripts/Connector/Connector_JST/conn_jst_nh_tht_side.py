...
orientation = 'H'
number_of_rows = 1
datasheet = 'http://www.jst-mfg.com/product/pdf/eng/eNH.pdf'
...
def generate_one_footprint(pincount, configuration):
    mpn = "BS{pincount}P-SHF-1AA".format(pincount=pincount)
    ...
    ########################### CrtYd #################################
    cx1 = roundToBase((x1-configuration['courtyard_offset']['connector'])-1.5, configuration['courtyard_grid'])
    cy1 = roundToBase((y1-configuration['courtyard_offset']['connector'])-5.5, configuration['courtyard_grid'])

    cx2 = roundToBase((x2+configuration['courtyard_offset']['connector'])+1.5, configuration['courtyard_grid'])
    ...
    for pin in range(pincount):
        x = pin * pitch
        kicad_mod.append(PolygoneLine(polygone=[{'x': x-0.5,'y': -3},
                                   {'x': x-0.5,'y': -8},
                                   {'x': x-0.3,'y': -8.5},
                                   {'x': x+0.3,'y': -8.5},
                                   {'x': x+0.5,'y': -8},
                                   {'x': x+0.5,'y': -3}], layer='F.SilkS', width=configuration['silk_line_width']))
...
if __name__ == "__main__":
    ...
    for pincount in range(2, 26):
        if pincount in [17, 19, 23]:
            continue
