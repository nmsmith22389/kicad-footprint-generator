...
orientation = 'V'
datasheet = "https://cdn.amphenol-icc.com/media/wysiwyg/files/drawing/62674.pdf"

pincounts = range(4, 41)
d_between_rows = 0.8        # [mm]
pad_height = 1.6            # [mm]
pad_width = 0.4             # [mm]
pad_pitch = 0.5             # [mm]
tab_height = pad_height     # [mm]
tab_width = 0.6             # [mm]
tab_pins_d = 1.0            # [mm]
housing_length = 2.95       # [mm]
...
def generate_one_footprint(pincount, configuration):
    ...
    footprint_name = 'Amphenol_{pn:s}_2Rows-{pc:02g}Pins-1MP_P0.5mm_Vertical'\
        .format(pn=partnumber, pc=pincount)
    print('Building {:s}'.format(footprint_name))

    # initialise footprint
    kicad_mod = Footprint(footprint_name)
    kicad_mod.setDescription('Amphenol FPC connector, {pc:02g} top-side contacts, 0.5mm pitch, SMT, {ds}'.format(pc=pincount, ds=datasheet))
    kicad_mod.setTags('Amphenol FPC {:s} Vertical Top'.format(partnumber))
    ...
    tab_x = pin_edge_offset - tab_pins_d
    tab_y = row_offset
    ...
    kicad_mod.append(PolygoneLine(
        polygone=[
            [-housing_x_offset, housing_y_offset - 0.55],
            [housing_x_offset, housing_y_offset - 0.55],
            [housing_x_offset, -housing_y_offset - 0.55],
            [-housing_x_offset, -housing_y_offset - 0.55],
            [-housing_x_offset, housing_y_offset - 0.55]],
    ...
    kicad_mod.append(PolygoneLine(
        polygone=[
            [pin_edge_offset - 0.4, housing_y_offset - 0.55],
            [pin_edge_offset, housing_y_offset - 0.8 - 0.55],
            [pin_edge_offset + 0.4, housing_y_offset - 0.55]],
        layer='F.Fab', width=configuration['fab_line_width']))
    ...
    kicad_mod.append(PolygoneLine(
        polygone=[
            [-housing_x_offset, housing_y_offset - 0.55],
            [tab_x - 0.5, housing_y_offset - 0.55],
            [tab_x - 0.5, housing_y_offset - 0.55 + pad_height / 2]],
