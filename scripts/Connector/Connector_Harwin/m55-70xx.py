...
series = 'M55-70'
series_long = 'Male Vertical Surface Mount Dual Row 1.27mm (0.05 inch) Pitch PCB Connector'
manufacturer = 'Harwin'
pitch = 1.27
datasheet = 'https://cdn.harwin.com/pdfs/M55-70X.pdf'
mpn = 'M55-70x{pincount:02g}42R'
padsize = [1.1, 0.8]
...
def gen_footprint(pinnum, manpart, configuration):
    ...
    for y in range(0, pinnum//2):
        ...
        kicad_mod.append(RectLine(start=[-0.635-0.1, origy-0.1], end=[-0.635+0.1, origy+0.1], width=configuration['fab_line_width'], layer="F.Fab"))
        kicad_mod.append(RectLine(start=[ 0.635-0.1, origy-0.1], end=[ 0.635+0.1, origy+0.1], width=configuration['fab_line_width'], layer="F.Fab"))
    ...
    f_outline = [
        ...
        {'x':-1.5, 'y':  (pitch*(pinnum))/4 + 1},
        {'x': 0.3, 'y':  (pitch*(pinnum))/4 + 1},
        {'x': 0.3, 'y':  (pitch*(pinnum))/4 + 0.4},
