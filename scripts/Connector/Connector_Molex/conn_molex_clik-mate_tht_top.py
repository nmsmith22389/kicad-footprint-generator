...
series = "CLIK-Mate"
series_long = 'CLIK-Mate Wire-to-Board Connectors'
manufacturer = 'Molex'
orientation = 'V'
number_of_rows = 1
datasheet = 'https://www.molex.com/pdm_docs/as/AS-502578-001-001.pdf'

pitch = 1.5
drill = 0.7 # from datasheet (drawing)
...
eng_mpn = '503159-{n:02d}00'
new_mpn_example = '503159{n:02d}00'
...
def generate_one_footprint(pincount, configuration):
    ...
    body_edge={
        'left':start_pos_x - 1.65 - 0.55,
        'right':end_pos_x + 1.65 + 0.55,
        'bottom':0.76
        }
    body_edge['top'] = body_edge['bottom'] - 6.55
    ...
    kicad_mod.append(PadArray(initial=1, increment=2, start=[start_pos_x, 0],
        x_spacing=2*pitch, pincount=pincount - floor(pincount/2),
        size=pad_size, drill=drill,
        type=Pad.TYPE_THT, shape=Pad.SHAPE_OVAL, layers=Pad.LAYERS_THT,
        **optional_pad_params))

    kicad_mod.append(PadArray(initial=2, increment=2, start=[start_pos_x + pitch, -2.00],
        x_spacing=2*pitch, pincount=floor(pincount/2),
        size=pad_size, drill=drill,
        type=Pad.TYPE_THT, shape=Pad.SHAPE_OVAL, layers=Pad.LAYERS_THT,
        **optional_pad_params))

    # boss (mounting pin)
    kicad_mod.append(Pad(at=[-1.65, -3.8],type=Pad.TYPE_NPTH, shape=Pad.SHAPE_CIRCLE, size=0.8, drill=0.8, layers=Pad.LAYERS_NPTH))
    ...
    # triangular pin 1 marker silkscreen
    tri = [{'x': start_pos_x, 'y': 0.92035},
            {'x': start_pos_x-0.35, 'y': 1.526568},
            {'x': start_pos_x+0.35, 'y': 1.526568},
            {'x': start_pos_x, 'y': 0.92035}]
    kicad_mod.append(PolygoneLine(polygone=tri, layer='F.SilkS', width=silk_w))

    # triangular pin 1 marker fabrication
    sl=1
    poly_pin1_marker_fab = [
        {'x': start_pos_x - 1.145, 'y': -3.64465},
        {'x': start_pos_x - 1.145, 'y': -4.34465},
        {'x': start_pos_x - 0.538782, 'y': -3.99465},
        {'x': start_pos_x - 1.145, 'y': -3.64465}
    ]
    kicad_mod.append(PolygoneLine(polygone=poly_pin1_marker_fab, layer='F.Fab', width=fab_w))
    ...
    if pincount < 3:
        corners = [{'x': centre_x, 'y': body_edge['bottom']-lower_border},
            {'x': body_edge['left'] + lateral_border, 'y': body_edge['bottom']-lower_border},
            {'x': body_edge['left'] + lateral_border, 'y': body_edge['top']+upper_border},
            {'x': body_edge['left'] + lateral_border + first_stretch, 'y': body_edge['top']+upper_border},
            {'x': body_edge['left'] + lateral_border + first_stretch, 'y': body_edge['top']+upper_border},
            {'x': body_edge['left'] + lateral_border + first_stretch, 'y': body_edge['top'] + upper_border + little_corner},
            {'x': body_edge['left'] + lateral_border + first_stretch + corner_width, 'y': body_edge['top'] + upper_border + little_corner},
            {'x': body_edge['left'] + lateral_border + first_stretch + corner_width, 'y': body_edge['top'] + upper_border},
            {'x': centre_x, 'y': body_edge['top'] + upper_border}
            ]
        ...
        corners = [{'x': centre_x, 'y': body_edge['bottom']-lower_border},
            ...
            {'x': body_edge['left'] + lateral_border + first_stretch, 'y': body_edge['top']+upper_border},
            {'x': body_edge['left'] + lateral_border + first_stretch, 'y': body_edge['top']+upper_border},
            {'x': body_edge['left'] + lateral_border + first_stretch, 'y': body_edge['top'] + upper_border + large_corner},
            {'x': body_edge['left'] + lateral_border + first_stretch + corner_width, 'y': body_edge['top'] + upper_border + large_corner},
            {'x': body_edge['left'] + lateral_border + first_stretch + corner_width, 'y': body_edge['top'] + upper_border},
            {'x': centre_x, 'y': body_edge['top'] + upper_border}
            ]
    else:
        corners = [{'x': centre_x, 'y': body_edge['bottom']-lower_border},
            {'x': body_edge['left'] + lateral_border, 'y': body_edge['bottom']-lower_border},
            {'x': body_edge['left'] + lateral_border, 'y': body_edge['top']+upper_border+large_offset},
            {'x': body_edge['left'] + lateral_border + first_stretch, 'y': body_edge['top']+upper_border+large_offset},
            {'x': body_edge['left'] + lateral_border + first_stretch, 'y': body_edge['top']+upper_border},
            {'x': body_edge['left'] + lateral_border + first_stretch + pitch, 'y': body_edge['top']+upper_border},
            {'x': body_edge['left'] + lateral_border + first_stretch + pitch, 'y': body_edge['top']+upper_border},
            {'x': body_edge['left'] + lateral_border + first_stretch + pitch, 'y': body_edge['top'] + upper_border + large_corner},
            {'x': body_edge['left'] + lateral_border + first_stretch + pitch + corner_width, 'y': body_edge['top'] + upper_border + large_corner},
            {'x': body_edge['left'] + lateral_border + first_stretch + pitch + corner_width, 'y': body_edge['top'] + upper_border},
            {'x': centre_x, 'y': body_edge['top'] + upper_border}
            ]

    kicad_mod.append(PolygoneLine(polygone=corners, layer='F.SilkS', width=silk_w))
    kicad_mod.append(PolygoneLine(polygone=corners, x_mirror=centre_x, layer='F.SilkS', width=silk_w))
