...
def generate_footprint(params, mpn, configuration):
    ...
    fp_name = "Mounting_Penn_{series}-{size_prefix}{size}_H{h}mm{td}{suffix}_{mpn}".format(
    ...
    kicad_mod.setDescription("Mounting Hardware, {hole_type} {size}, height {h}, Penn Engineering {mpn} ({ds:s}), generated with kicad-footprint-generator".format(size=size, h=h, mpn=mpn, ds=part_params['datasheet'], hole_type=hole_type))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--params', type=str, nargs='?', help='the part definition file', default='./penn_smt_spacer.yaml')
